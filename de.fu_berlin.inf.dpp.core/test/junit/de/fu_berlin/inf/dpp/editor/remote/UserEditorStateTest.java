...
public class UserEditorStateTest {
    ...
    public void testFreshUserState() {
        assertNotNull("fresh user state should have an empty list of editors",
            state.getOpenEditors());
        assertNull("fresh user state should have no active editor",
            state.getActiveEditorState());
    }

    @Test
    public void testIgnoreActivitiesOnActivatedEditors() {
        assertFalse("user state should not exist before activity reception",
            state.getOpenEditors().contains(pathA));

        select(pathA, 5, 15);
        view(pathA, 10, 30);

        assertFalse("user state should not exist after awareness information "
            + "on non-activated editor", state.getOpenEditors().contains(pathA));

        activate(pathA);

        assertTrue("user state should exist after editor was opened", state
            .getOpenEditors().contains(pathA));
    }
    ...
    public void testUpdateViewports() {
        ...
        assertEquals("viewport length should be updated", 45,
            v.getNumberOfLines());
    }
    ...
    public void testActivateAndClose() {
        ...
        assertEquals("both activated editors should be open", 2, state
            .getOpenEditors().size());

        assertEquals("last activated editor should be active", pathB, state
            .getActiveEditorState().getPath());

        activate(pathA);

        assertEquals("last activated editor should be active", pathA, state
            .getActiveEditorState().getPath());

        close(pathA);

        assertNull("no editor should be active after the active editor "
            + "was closed", state.getActiveEditorState());
        assertEquals("only one editor should be open", 1, state
            .getOpenEditors().size());
    }
    ...
    private void select(SPath path, int offset, int length) {
        state.consumer.exec(new TextSelectionActivity(source, offset, length,
            path));
    }
