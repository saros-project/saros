...
public class FileListTest {
    ...
    public void testCreateFileListForProject() throws IOException {
        ...
        assertTrue("file list does not contain file: info.txt",
            paths.contains("info.txt"));

        assertTrue("file list does not contain file: foobar/info.txt",
            paths.contains("foobar/info.txt"));

        assertTrue("file list does not contain folder: bar",
            paths.contains("bar/"));

        assertTrue("file list does not contain folder: foobar/foo",
            paths.contains("foobar/foo/"));

        final Set<String> expectedEncodings = new HashSet<String>(
            Arrays.asList("ISO-8859-1", "UTF-8", "UTF-16"));

        assertNotNull("no checksum found for file: info.txt",
            fileList.getMetaData("info.txt"));

        assertNotNull("no checksum found for file: foobar/info.txt",
            fileList.getMetaData("foobar/info.txt"));

        assertEquals("not all encodings were fetched", expectedEncodings,
            fileList.getEncodings());
    }
    ...
    private static IProject createProjectLayout() {
        ...
        final IFolder barFolder = createFolderMock(project, "bar",
            new IResource[0]);

        final IFolder foobarfooFolder = createFolderMock(project, "foobar/foo",
            new IResource[0]);

        final IFile infoTxtFile = createFileMock(project, "info.txt", "1234",
            "UTF-8");

        final IFile foobarInfoTxtFile = createFileMock(project,
            "foobar/info.txt", "12345", "ISO-8859-1");

        final IFolder foobarFolder = createFolderMock(project, "foobar",
            new IResource[] { foobarfooFolder, foobarInfoTxtFile });

        EasyMock.expect(project.getName()).andStubReturn("foo");

        try {
            EasyMock.expect(project.getDefaultCharset())
                .andStubReturn("UTF-16");

            EasyMock.expect(project.members()).andStubReturn(
                new IResource[] { barFolder, infoTxtFile, foobarFolder });
        } catch (IOException e) {
            // cannot happen
        }
    ...
    private static IFile createFileMock(final IProject project,
        final String path, final String content, final String encoding) {
    ...
    private static IFolder createFolderMock(final IProject project,
        final String path, final IResource[] members) {
