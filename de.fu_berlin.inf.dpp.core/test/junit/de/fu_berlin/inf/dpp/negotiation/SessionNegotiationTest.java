...
public class SessionNegotiationTest {
    ...
    public void setup() throws Exception {
        FakeConnectionFactoryResult result = FakeConnectionFactory
            .createConnections(ALICE, BOB).withStrictJIDLookup()
            .withThreadedReceiver().get();
        ...
        aliceVersionManager = new VersionManager("47.11.8015.TEST",
            aliceReceiver, aliceTransmitter);

        bobVersionManager = new VersionManager("47.11.8015.TEST", bobReceiver,
            bobTransmitter);
    ...
    private DiscoveryManager discoveryManagerMockFor(final JID jid) {
        ...
        expect(
            discoManagerMock.getSupportingPresence(eq(jid),
                anyObject(String.class))).andStubReturn(jid);
    ...
    public void testGenericSessionNegotiation() throws Throwable {
        ...
        final OutgoingSessionNegotiation aliceOut = new OutgoingSessionNegotiation(
            BOB, "", aliceSessionManager, aliceSession,
            new SessionNegotiationHookManager(), aliceVersionManager,
            aliceDiscoveryManager, aliceTransmitter, aliceReceiver);
        ...
        aliceNegotiationThread = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    if (status != Status.OK)
                        throw new AssertionError("OSN failed: " + status + ", "
                            + aliceOut.getErrorMessage());
        ...
        final Thread negotiationAwaitThread = new Thread(new Runnable() {
            ...
            @Override
            public void run() {
                PacketCollector collector = bobReceiver
                    .createCollector(InvitationOfferingExtension.PROVIDER
                        .getPacketFilter());

                bobNegotiationStart.countDown();
                Packet packet = collector.nextResult(30000);

                final InvitationOfferingExtension payload = InvitationOfferingExtension.PROVIDER
                    .getPayload(packet);
                ...
                final IncomingSessionNegotiation bobIn = new IncomingSessionNegotiation(
                    ALICE, negotiationID, sessionID,
                    "version not used by the negotiation itself", description,
                    bobSessionManager, new SessionNegotiationHookManager(),
                    bobConnectionManager, bobTransmitter, bobReceiver);

                bobNegotiationThread = new Thread(new Runnable() {
                    @Override
                    public void run() {
                        try {
                            ...
                            if (status != Status.OK)
                                throw new AssertionError("ISN failed: "
                                    + status + ", " + bobIn.getErrorMessage());
        ...
        if (bobNegotiationThread == null)
            fail("bobs negotiation thread did not start");
