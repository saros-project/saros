 ...
 * PicoContainer full of mocked components.
 * <p>
...
public class ContextMocker {
    ...
    /**
     * Creates an empty container which can be equipped with dependency mappings
     * (e.g. "key -> implementation") in various ways.
     * 
     ...
     * 
     * @param container
     *            target for the dependency to mock
     * @param clazz
     *            Class for which a {@linkplain PowerMock#createNiceMock(Class)
     *            nice mock} will be created. Must not add the same key twice
     *            (neither through this nor through the other add methods).
     */
    public static <T> void addMock(MutablePicoContainer container,
        Class<T> clazz) {
     ...
     * 
     * @param container
     *            target for the dependencies to mock
     * @param classes
     *            Classes for which a
     *            {@linkplain PowerMock#createNiceMock(Class) nice mock} will be
     *            created. Must not add the same key twice (neither through this
     *            nor through the other add methods).
     */
    public static void addMocks(MutablePicoContainer container,
        List<Class<?>> classes) {
    ...
    /**
     * Reads all dependencies that a context factory defines and creates mocks
     * for all of them.
     * 
     * @param container
     *            target container for the dependencies to mock
     * @param factory
     *            Context factory from which all component dependencies (i.e. no
     *            {@link BindKey}s) are extracted to create mock instances for
     *            all of them. Must not add the same key twice (neither through
    ...
    private static class CollectingPicoContainer extends DefaultPicoContainer {
        ...
        @Override
        public MutablePicoContainer addComponent(Object key, Object value,
            Parameter... params) {
