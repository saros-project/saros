...
public class ActivitySequencerTest {
    ...
    public void setUp() {
        ...
        aliceUser = new User(ALICE_JID, null, true, true, 0, 0);
        bobUser = new User(BOB_JID, null, false, true, 0, 0);
        ...
        FakeConnectionFactoryResult result = FakeConnectionFactory
            .createConnections(ALICE_JID, BOB_JID).withStrictJIDLookup().get();
    ...
    public void tearDown() {
        if (aliceSequencer != null)
            aliceSequencer.stop();

        if (bobSequencer != null)
            bobSequencer.stop();
    ...
    public void testUnregisterUserOnTransmissionFailure() {
        ...
        try {
            brokenTransmitter.send(EasyMock.anyObject(String.class),
        ...
        User bobUserInAliceSession = new User(BOB_JID, null, false, false, 0, 0);
    ...
    public void testSendAndFlushAndReceiveAndOrder() {
        ...
        User bobUserInAliceSession = new User(BOB_JID, null, false, false, 0, 0);
        User aliceUserInBobSession = new User(ALICE_JID, null, true, false, 0,
            0);
