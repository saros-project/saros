...
/**
 * A BasicComponentParameter should be used to pass in a particular component as
 * argument to a different component's constructor. This is particularly useful
 * in cases where several components of the same type have been registered, but
 * with a different key. Passing a ComponentParameter as a parameter when
 * registering a component will give PicoContainer a hint about what other
 * component to use in the constructor. This Parameter will never resolve
 * against a collecting type, that is not directly registered in the
 * PicoContainer itself.
 * 
...
public class BasicComponentParameter extends AbstractParameter implements
    Parameter, Serializable {

    /**
     * <code>BASIC_DEFAULT</code> is an instance of BasicComponentParameter
     ...
     *             {@inheritDoc}
     * @see Parameter#isResolvable(PicoContainer, ComponentAdapter, Class,
    ...
    public Resolver resolve(final PicoContainer container,
        ...
        if (!(expectedType instanceof Class)) {
            if (expectedType instanceof ParameterizedType) {
                resolvedClassType = (Class<?>) ((ParameterizedType) expectedType)
                    .getRawType();
            } else {
        ...
        if (injecteeAdapter == null) {
            componentAdapter0 = resolveAdapter(container, forAdapter,
                resolvedClassType, expectedNameBinding, useNames, binding);
        } else {
        ...
        return new Resolver() {
            ...
            public Object resolveInstance() {
                ...
                if (componentAdapter instanceof DefaultPicoContainer.LateInstance) {
                    return convert(getConverters(container),
                        ((DefaultPicoContainer.LateInstance) componentAdapter)
                            .getComponentInstance(), expectedType);
                    ...
                    return convert(getConverters(container),
                        container.getComponent(
                            ...
                            makeInjectInto(forAdapter)), expectedType);
                }
            }
    ...
    private Converters getConverters(PicoContainer container) {
        return container instanceof Converting ? ((Converting) container)
            .getConverters() : null;
    }

    private static InjectInto makeInjectInto(ComponentAdapter<?> forAdapter) {
        return new InjectInto(forAdapter.getComponentImplementation(),
            forAdapter.getComponentKey());
    }

    private static Object convert(Converters converters, Object obj,
        Type expectedType) {
    ...
    public void verify(PicoContainer container, ComponentAdapter<?> forAdapter,
        Type expectedType, NameBinding expectedNameBinding, boolean useNames,
        Annotation binding) {
        final ComponentAdapter componentAdapter = resolveAdapter(container,
            forAdapter, (Class<?>) expectedType, expectedNameBinding, useNames,
            binding);
        if (componentAdapter == null) {
    ...
    protected <T> ComponentAdapter<T> resolveAdapter(PicoContainer container,
        ...
        if (componentKey != null) {
            ...
            result = typeComponentAdapter(container
            ...
            ComponentAdapter byKey = container
                .getComponentAdapter(expectedType);
            ...
            if (result == null && useNames) {
                ComponentAdapter found = container
                    .getComponentAdapter(expectedNameBinding.getName());
                if ((found != null)
                    && areCompatible(container, expectedType, found)
                    && found != adapter) {
                    result = found;
                }
            }
            ...
            if (result == null) {
                List<ComponentAdapter<T>> found = binding == null ? container
                    .getComponentAdapters(expectedType) : container
                    .getComponentAdapters(expectedType,
                        binding.annotationType());
                removeExcludedAdapterIfApplicable(excludeKey, found);
                if (found.size() == 0) {
                    result = noMatchingAdaptersFound(container, expectedType,
                        expectedNameBinding, binding);
        ...
        if (!type.isAssignableFrom(result.getComponentImplementation())) {
            ...
            if (!(result.getComponentImplementation() == String.class && getConverters(
                container).canConvert(type))) {
                return null;
            }
        }
        return result;
    ...
    private <T> ComponentAdapter<T> noMatchingAdaptersFound(
        PicoContainer container, Class<T> expectedType,
    ...
    private <T> AbstractInjector.AmbiguousComponentResolutionException tooManyMatchingAdaptersFound(
        ...
        AbstractInjector.AmbiguousComponentResolutionException exception = new AbstractInjector.AmbiguousComponentResolutionException(
            expectedType, foundClasses);
        return exception;
    }

    private <T> void removeExcludedAdapterIfApplicable(Object excludeKey,
    ...
    private <T> boolean areCompatible(PicoContainer container,
        Class<T> expectedType, ComponentAdapter found) {
        ...
        // stringConverters.containsKey(expectedType)) ;
            (foundImpl == String.class && getConverters(container) != null && getConverters(
                container).canConvert(expectedType));
    }
}
