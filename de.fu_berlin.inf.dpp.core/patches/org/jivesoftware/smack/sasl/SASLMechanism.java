 ...
 *
 * <ul>
 *   <li>{@link #getName()} -- returns the common name of the SASL mechanism.
 * </ul>
 *
 ...
 * <li>{@link #challengeReceived(String)} -- Handle a challenge from the server.
 * </ul>
 *
 * Basic XMPP SASL authentication steps: 1. Client authentication initialization, stanza sent to the
 * server (Base64 encoded): <auth xmlns='urn:ietf:params:xml:ns:xmpp-sasl' mechanism='DIGEST-MD5'/>
 * 2. Server sends back to the client the challenge response (Base64 encoded) sample: realm=<sasl
 * server realm>,nonce="OA6MG9tEQGm2hh",qop="auth",charset=utf-8,algorithm=md5-sess 3. The client
 * responds back to the server (Base 64 encoded): sample: username=<userid>,realm=<sasl server realm
 * from above>,nonce="OA6MG9tEQGm2hh", cnonce="OA6MHXh6VqTrRk",nc=00000001,qop=auth,
 * digest-uri=<digesturi>, response=d388dad90d4bbd760a152321f2143af7, charset=utf-8, authzid=<id> 4.
 * The server evaluates if the user is present and contained in the REALM if successful it sends:
 * <response xmlns='urn:ietf:params:xml:ns:xmpp-sasl'/> (Base64 encoded) if not successful it sends:
 * sample: <challenge xmlns='urn:ietf:params:xml:ns:xmpp-sasl'>
...
public abstract class SASLMechanism implements CallbackHandler {
   ...
   * xmpp/serverName From RFC-2831: digest-uri = "digest-uri" "=" digest-uri-value digest-uri-value
  ...
  /**
   * Same as {@link #authenticate(String, String, String, String)}, but with the hostname used as
   * the serviceName.
   *
   * <p>Kept for backward compatibility only.
   *
   * @param username the username of the user being authenticated.
   * @param host the hostname where the user account resides.
   * @param password the password for this account.
