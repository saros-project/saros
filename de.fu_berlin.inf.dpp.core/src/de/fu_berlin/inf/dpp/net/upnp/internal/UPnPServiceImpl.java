...
public class UPnPServiceImpl implements IUPnPService, Disposable {
    ...
    private class MappingRefreshTask extends TimerTask {
        ...
        public MappingRefreshTask(final GatewayDevice device, final int port,
    ...
    @Override
    public synchronized boolean createPortMapping(final GatewayDevice device,
        final int port, final String protocol, String description) {
        ...
        try {
            ...
            PortMappingEntry portMapping = upnpAccess
                .getSpecificPortMappingEntry(device, port, protocol);

            if (portMapping != null
                && !description.equals(portMapping.getPortMappingDescription()))
            ...
            int errorcode = upnpAccess.addPortMapping(device, port, port,
                localAddress.getHostAddress(), protocol, description,
                MAPPINGLEASEDURATION);

            // in case mapping with lease duration fails, try without
            if (errorcode != 0) {
                isPersistentMapping = true;
                errorcode = upnpAccess.addPortMapping(device, port, port,
                    localAddress.getHostAddress(), protocol, description,
                    /* Persistent */0);
            }

            if (errorcode != 0) {
                throw new IOException("device " + device.getFriendlyName()
                    + " replied with error code=" + errorcode);
        ...
        Map<String, Set<Integer>> mappedPortsForDevice = currentMappedPorts
            .get(device);
        ...
        Set<Integer> mappedPortsForProtocol = mappedPortsForDevice
            .get(protocol);
        ...
        if (!isPersistentMapping) {
            ...
            mappingRefreshTimer.schedule(new MappingRefreshTask(device, port,
                protocol, description), MAPPINGLEASEDURATION * 1000);
        }
    ...
    @Override
    public boolean deletePortMapping(final GatewayDevice device,
        ...
        try {
            ...
            boolean success = upnpAccess.deletePortMapping(device, port,
                protocol) == 0;
    ...
    public boolean isMapped(GatewayDevice device, int port, String protocol) {
        final Map<String, Set<Integer>> mappedPortsForDevice = currentMappedPorts
            .get(device);

        if (mappedPortsForDevice == null)
            return false;

        final Set<Integer> mappedPortsForProtocol = mappedPortsForDevice
            .get(protocol);
    ...
    public InetAddress getExternalAddress(GatewayDevice device) {
        ...
        try {
            ...
            LOG.warn(
                "failed to discover external address of device: "
                    + device.getFriendlyName(), e);
            return null;
        }
