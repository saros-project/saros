...
public class MDNSService {
    ...
    private final Map<String, ServiceInfo> resolvedDNSService = Collections
        .synchronizedMap(new HashMap<String, ServiceInfo>());

    private final List<ServiceListener> serviceListeners = new CopyOnWriteArrayList<ServiceListener>();

    private final ServiceListener forwarder = new ServiceListener() {
     ...
     * @param properties
     *            properties describing the service or <code>null</code>
     * @throws IllegalStateException
     *             if the service is already running.
     */
    public synchronized void configure(String type, String name, int port,
        Map<String, String> properties) {
        if (running)
            throw new IllegalStateException(
                "service cannot be configured while it is running");
        ...
        if (properties != null) {
            for (Entry<String, String> entry : properties.entrySet())
                currentServiceProperties.put(entry.getKey(), entry.getValue()
                    .getBytes(CHARSET));
        }
    ...
    public synchronized void start() throws IOException {
        if (!configured)
            throw new IOException("service is not configured");
        ...
        final ServiceInfo serviceInfo = ServiceInfo.create(currentServiceType,
            currentServiceName, currentServicePort, 0, 0, true,
    ...
    /**
     * Returns the fully qualified service name such as
     * <code>foobar._http._tcp.local.</code> after the service has been started.
     * 
     * @return the fully qualified service name or <code>null</code> if the
    ...
    /**
     * Adds a service listener. The type of the service that is listened for is
     * determined when the service is configured.
     * 
     * @param listener
     *            the listener to add
