 ...
 * <p>
 * See http://xmpp.org/extensions/xep-0047.html for more details.
 */
public class IBBStreamService implements IStreamService, BytestreamListener {
    ...
    public IByteStreamConnection connect(String connectionID, JID remoteAddress)
        ...
        try {
            session = manager.establishSession(remoteAddress.toString(),
                connectionID);
        } catch (XMPPException e) {
            throw new IOException(e);
        }

        return new BinaryChannelConnection(localAddress, remoteAddress,
            connectionID, new XMPPByteStreamAdapter(session), StreamMode.IBB,
    ...
    public void incomingBytestreamRequest(BytestreamRequest request) {
        ...
        try {
            ...
            LOG.error(
                "failed to accept IBB bytestream from: " + request.getFrom(), e);
            return;
            ...
            LOG.error("interrupted while accepting IBB bytestream from: "
                + request.getFrom(), e);
            return;
        }
        ...
        try {
            connection = new BinaryChannelConnection(localAddress, new JID(
                request.getFrom()), request.getSessionID(),
                new XMPPByteStreamAdapter(session), StreamMode.IBB,
                connectionListener);
        } catch (IOException e) {
        ...
        connectionListener.connectionChanged(request.getSessionID(),
            connection, true);
    }
