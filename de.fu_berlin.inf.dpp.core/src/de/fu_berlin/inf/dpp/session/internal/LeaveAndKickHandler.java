...
/**
 * A session component which listens for leave and kick messages from the
 * session's host and stops the session in response.
 */
public class LeaveAndKickHandler implements Startable {
     ...
     *            the session manager
     * @param receiver
     *            the receiver for listening to messages
     */
    public LeaveAndKickHandler(ISarosSession session,
        ISarosSessionManager sessionManager, IReceiver receiver) {
    ...
    private void leaveReceived(JID from) {
        ...
        if (!session.isHost()) {
            LOG.warn("Received leave message from user " + user
                + " who is not the current session's host");
            return;
        }
