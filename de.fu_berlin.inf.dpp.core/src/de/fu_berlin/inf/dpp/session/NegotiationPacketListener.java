 ...
 * network messages (packets) to the Saros session
 * {@linkplain SarosSessionManager manager}.
 * <p>
 * <b>Restriction:</b> This class must only instantiated by the
 * <code>SarosSessionManager</code> itself.
 */
final class NegotiationPacketListener {
    ...
    private final ISessionLifecycleListener sessionLifecycleListener = new NullSessionLifecycleListener() {
        ...
        public void sessionStarted(final ISarosSession session) {
            receiver.addPacketListener(projectNegotiationRequestListener,
                ProjectNegotiationOfferingExtension.PROVIDER
                    .getPacketFilter(session.getID()));

            receiver.addPacketListener(projectNegotiationCanceledListener,
     ...
     */
    private final PacketListener sessionNegotiationCanceledListener = new PacketListener() {

        @Override
        public void processPacket(final Packet packet) {

            final CancelInviteExtension extension = CancelInviteExtension.PROVIDER
                .getPayload(packet);

            if (extension == null) {
                LOG.warn("received malformed session negotiation packet from "
                    + packet.getFrom());
                return;
            }

            sessionNegotiationCanceled(new JID(packet.getFrom()),
                extension.getNegotiationID(), extension.getErrorMessage());
        }
    };

    private final PacketListener sessionNegotiationRequestListener = new PacketListener() {

        @Override
        public void processPacket(final Packet packet) {

            final InvitationOfferingExtension extension = InvitationOfferingExtension.PROVIDER
                .getPayload(packet);
            ...
            sessionNegotiationRequest(new JID(packet.getFrom()),
                extension.getNegotiationID(), extension.getVersion(),
                extension.getSessionID(), extension.getDescription());

        }
    };

    private final PacketListener projectNegotiationCanceledListener = new PacketListener() {

        @Override
        public void processPacket(Packet packet) {

            final CancelProjectNegotiationExtension extension = CancelProjectNegotiationExtension.PROVIDER
                .getPayload(packet);

            if (extension == null) {
                LOG.warn("received malformed project negotiation packet from "
                    + packet.getFrom());
                return;
            }
    ...
    private final PacketListener projectNegotiationRequestListener = new PacketListener() {

        @Override
        public void processPacket(final Packet packet) {

            final ProjectNegotiationOfferingExtension extension = ProjectNegotiationOfferingExtension.PROVIDER
                .getPayload(packet);
    ...
    private void init() {
        receiver.addPacketListener(sessionNegotiationCanceledListener,
    ...
    private void sessionNegotiationRequest(final JID sender,
        final String negotiationID, final String remoteVersion,
        final String sessionID, final String description) {

        LOG.info("received invitation from " + sender + " [negotiation id: "
            + negotiationID + ", " + "session id: " + sessionID + ", "
            + "version: " + remoteVersion + "]");

        if (rejectSessionNegotiationRequests) {
            LOG.info("rejecting session negotiation request with id: "
                + negotiationID);
             ...
             * logic here.
             */
            final PacketExtension response = CancelInviteExtension.PROVIDER
                .create(new CancelInviteExtension(negotiationID,
        ...
        final PacketExtension response = InvitationAcknowledgedExtension.PROVIDER
            .create(new InvitationAcknowledgedExtension(negotiationID));
