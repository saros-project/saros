...
public class UserEditorState {
    ...
    final IActivityConsumer consumer = new AbstractActivityConsumer() {
        ...
        public void receive(TextSelectionActivity textSelectionActivity) {
            ...
            if (!openEditors.containsKey(path)) {
                LOG.warn("received selection for editor which was never activated: "
                    + path);
                return;
            }
     ...
     * 
     * @param path
     *            The path of the file for which the editor state should be
     *            retrieved
     * 
     * @return An immutable {@link EditorState}, or <code>null</code> if there
     *         is no open editor for the given path (i.e. it's not in
     *         {@link #getOpenEditors()})
     */
    public synchronized EditorState getEditorState(SPath path) {
        ...
        return new EditorState(result.getPath(), result.getViewport(),
            result.getSelection());
    }
     ...
     * 
     * @return An immutable {@link EditorState}, or <code>null</code> if the
     *         user has no currently active editor
     */
    public synchronized EditorState getActiveEditorState() {
        if (activeEditor == null)
            return null;

        return new EditorState(activeEditor.getPath(),
            activeEditor.getViewport(), activeEditor.getSelection());
    }
