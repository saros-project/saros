...
public class UserEditorStateManager implements IActivityConsumer, Startable {
    ...
    /**
     * Most sessions have two participants, some have three. So there are one,
     * maybe two remote users (<code>initialCapacity: 2</code>). Furthermore,
     * mostly expect access from one thread, maybe two (
     * <code>concurrencyLevel: 2</code>).
     */
    private final ConcurrentHashMap<User, UserEditorState> userEditorStates = new ConcurrentHashMap<User, UserEditorState>(
        2, 0.75f, 2);
     ...
     * 
     * @param sarosSession
     *            the session to listen on for activities and users leaving and
     *            joining
     */
    ...
    /**
     * Retrieve the status of the shared editors of the given remote user as
     * inferred by the activities received from him/her.
     * 
     * @param user
     *            A user in the current session
     * @return The remote user's current status of shared editors. In case we
     *         didn't receive any activities from this user yet, the status will
     *         be empty (see {@link UserEditorState}).
     */
    ...
    /**
     * Returns all paths representing the editors which are currently opened by
     * the remote users of the running session (i.e. not our own).
     * 
     * @return A set containing the paths corresponding to all open editors. If
     *         no editors are opened an empty set is being returned.
     */
