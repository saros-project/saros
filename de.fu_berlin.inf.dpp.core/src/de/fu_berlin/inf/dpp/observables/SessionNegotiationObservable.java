...
public final class SessionNegotiationObservable {
     ...
     * @param id
     *            the ID to lookup
     * @return the current {@link SessionNegotiation session negotiation} or
     *         <code>null</code> if no such negotiation exists
     */
    public synchronized SessionNegotiation get(final JID jid, final String id) {
        final List<SessionNegotiation> currentNegotiations = negotiations
            .get(jid);
    ...
    public synchronized void add(final SessionNegotiation negotiation) {
        ...
        for (final SessionNegotiation currentNegotiation : currentNegotiations) {
            if (currentNegotiation.getID().equals(negotiation.getID())) {
                LOG.warn("a session negotiation with ID " + negotiation.getID()
