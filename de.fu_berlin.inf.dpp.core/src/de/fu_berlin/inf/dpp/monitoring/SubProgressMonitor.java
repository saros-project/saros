...
public final class SubProgressMonitor implements IProgressMonitor {
    ...
    /**
     * Indicates that strings passed to {@linkplain #setTaskName},
     * {@linkplain #subTask}, and {@linkplain #beginTask} should all be
     * propagated to the parent.
     */
    public static final int SUPPRESS_NONE = 0;

    /**
     * Indicates that strings passed into {@linkplain #beginTask} should be
     * ignored.
     */
    public static final int SUPPRESS_BEGINTASK = 1;

    /**
     * Indicates that strings passed into {@linkplain #setTaskName} should be
     * ignored.
     */
     ...
     *            the number of work
     * @param style
     *            the style to use when changing labels which can be a
     *            combination of {@linkplain #SUPPRESS_BEGINTASK} and
     *            {@linkplain #SUPPRESS_SETTASKNAME} and
     *            {@linkplain #SUPPRESS_SUBTASK}, or only
     *            {@linkplain #SUPPRESS_NONE} if nothing should be suppressed
     * @throws IllegalArgumentException
     *             if monitor is <code>null</code>
     */
    public SubProgressMonitor(final IProgressMonitor monitor, final int ticks,
    ...
    public void setTaskName(final String name) {
        if ((style & SUPPRESS_SETTASKNAME) != 0)
            return;
    ...
    public void worked(final int amount) {
        if (amount <= 0 || totalWork == 0 || totalTicksToConsume == 0
            || !beginTaskCalled)
            return;
        ...
        final long newProgress = (currentWork * totalTicksToConsume)
            / totalWork;
