...
public class TestSaros extends Saros {
    ...
    public IPreferenceStore getPreferenceStore() {
        if (_preferenceStore == null) {
            ...
            _preferenceStore.setValue(
                PreferenceConstants.LOCAL_SOCKS5_PROXY_DISABLED, true);
            _preferenceStore.setValue(
                PreferenceConstants.FORCE_FILETRANSFER_BY_CHAT, true);
            _preferenceStore.setValue(
                PreferenceConstants.DISABLE_VERSION_CONTROL, true);
            _preferenceStore.setValue(PreferenceConstants.CHATSERVER,
                "conference.127.0.0.1");
        }
    ...
    public Preferences getConfigPrefs() {
        if (configPrefs == null)
            configPrefs = new MemoryPreferences();
    ...
    public void setXMPPUser(XmppUser user) {
        ...
        preferenceStore.setValue(PreferenceConstants.SERVER,
            user.getServerAdress());
        try {
            getSecurePrefs().put(PreferenceConstants.SERVER,
                user.getServerAdress(), false);
            getSecurePrefs().put(PreferenceConstants.USERNAME,
                user.getUsername(), false);
            getSecurePrefs().put(PreferenceConstants.PASSWORD,
