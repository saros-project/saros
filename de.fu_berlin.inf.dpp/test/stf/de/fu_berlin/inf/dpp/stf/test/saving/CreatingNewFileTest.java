...
public class CreatingNewFileTest extends StfTestCase {
    ...
    public static void runBeforeClass() throws RemoteException,
        ...
        setUpWorkbench();
        setUpSaros();
        Util.setUpSessionWithAJavaProjectAndAClass(CARL, BOB, ALICE);
    }
    ...
    public void testCarlCreateANewFile() throws IOException, CoreException {
        CARL.superBot().views().packageExplorerView().selectProject(Constants.PROJECT1)
            .newC().folder(Constants.FOLDER1);
        CARL.superBot().views().packageExplorerView()
            .selectFolder(Constants.PROJECT1, Constants.FOLDER1).newC().file(Constants.FILE1);
        ALICE.superBot().views().packageExplorerView()
            .waitUntilFileExists(Constants.PROJECT1, Constants.FOLDER1, Constants.FILE1);
        assertTrue(ALICE.superBot().views().packageExplorerView()
            .selectFolder(Constants.PROJECT1, Constants.FOLDER1).existsWithRegex(Constants.FILE1));
    ...
    public void testCarlGrantWriteAccess() throws IOException, CoreException,
        ...
        ALICE.superBot().views().packageExplorerView().selectProject(Constants.PROJECT1)
            .newC().folder(Constants.FOLDER2);
        ALICE.superBot().views().packageExplorerView()
            .selectFolder(Constants.PROJECT1, Constants.FOLDER2).newC().file(Constants.FILE2);

        CARL.superBot().views().packageExplorerView()
            .waitUntilFileExists(Constants.PROJECT1, Constants.FOLDER2, Constants.FILE2);
        assertTrue(CARL.superBot().views().packageExplorerView()
            .selectFolder(Constants.PROJECT1, Constants.FOLDER2).existsWithRegex(Constants.FILE2));
        BOB.superBot().views().packageExplorerView()
            .waitUntilFileExists(Constants.PROJECT1, Constants.FOLDER2, Constants.FILE2);
        assertTrue(BOB.superBot().views().packageExplorerView()
            .selectFolder(Constants.PROJECT1, Constants.FOLDER2).existsWithRegex(Constants.FILE2));
        ...
        String file2ContentOfAlice = ALICE.remoteBot().editor(Constants.FILE2).getText();

        CARL.remoteBot().editor(Constants.FILE2).waitUntilIsTextSame(file2ContentOfAlice);
        String file2ContentOfCarl = CARL.remoteBot().editor(Constants.FILE2).getText();
        assertTrue(file2ContentOfAlice.equals(file2ContentOfCarl));

        BOB.remoteBot().editor(Constants.FILE2).waitUntilIsTextSame(file2ContentOfAlice);
        String file2ContentOfBob = BOB.remoteBot().editor(Constants.FILE2).getText();
