...
/**
 * Implementation of the IWhiteboardFigure providing figure information being
 * necessary for testing.
 */
public class WhiteboardFigure extends StfRemoteObject implements
    IWhiteboardFigure {
    ...
    /**
     * initializes this WhiteboardFigure object with the values from the passed
     * IFigure.
     * 
     * @param figure
     */
    public void init(final IFigure figure) {
        Point location = new Point(figure.getBounds().getLocation().x, figure
            .getBounds().getLocation().y);
        Dimension size = new Dimension(figure.getBounds().getSize().width,
            figure.getBounds().getSize().height);
        ...
        backgroundColor = UIThreadRunnable.syncExec(new Result<Color>() {
            ...
            public Color run() {
                org.eclipse.swt.graphics.Color color = figure
                    .getBackgroundColor();
                return new Color(color.getRed(), color.getGreen(), color
                    .getRed());
            }
        });
