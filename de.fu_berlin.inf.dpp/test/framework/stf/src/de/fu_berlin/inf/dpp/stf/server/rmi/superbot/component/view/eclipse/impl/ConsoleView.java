...
public final class ConsoleView extends StfRemoteObject implements IConsoleView {
    ...
    public void clearCurrentConsole() throws RemoteException {
        ...
        SWTBotToolbarButton clearConsoleButton = view
            .toolbarButton("Clear Console");

        if (clearConsoleButton.isEnabled())
            clearConsoleButton.click();
    }

    @Override
    public void waitUntilCurrentConsoleContainsText(final String text)
    ...
    private SWTBotStyledText getCurrentConsoleTextWidget() {
        ...
        final List<? extends StyledText> styledTexts = view.bot().widgets(
            allOf(widgetOfType(StyledText.class)), view.getWidget());

        final StyledText widget = UIThreadRunnable
            .syncExec(new WidgetResult<StyledText>() {
        ...
        if (widget == null)
            throw new WidgetNotFoundException(
                "the console window does not contain an active console");
