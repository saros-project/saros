 ...
 * @author rdjemili
 */
public class ActivitySequencer implements IRequestManager, RequestForwarder,IActivitySequencer, IActivityManager {
...
//			// HOST SIDE
//			if (concurrentManager != null && concurrentManager.isHostSide()) {
//				/* concurrent control */
...
//			}
	...
	public void activityCreated(IActivity activity) {
		...
		/** 
		 * host activity: 
		 * 				put into outgoing queue and send to all if
		 * 				activity is generated by host. 
		 * otherwise:
		 * 			send request to host.
		 * */
//		if(concurrentManager.isHost()){
...
//		}
	...
	public synchronized void forwardOutgoingRequest(Request req) {
		 ...
		 *  send to jupiter server to sync with proxies.
		 */
		if(concurrentManager.isHost(req.getJID())){
			/* send req to jupiter document server*/
			/*Wichtig über sideID kann ermittelt werden, ob es sich um 
			 * eine client oder server nachricht vom host handelt!*/
//			if(req.getSiteId()== 1){
...
//		logger.debug("add request to outgoing queue : "+req.getJID()+" "+req.getOperation());
	...
	public IActivity receiveRequest(Request request) {
		/* sync with jupiter server on host side and transform operation with jupiter
		 * client side.*/
