...
public class CreateXMPPAccountWizardPage extends WizardPage {
    ...
    private Composite createLeftColumn(Composite composite) {
        ...
        useNowButton.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
            false));
    ...
    private void hookListeners() {
        ...
        useNowButton.addSelectionListener(new SelectionAdapter() {
            @Override
    ...
    private void updatePageCompletion() {
        ...
        boolean passwordsMatch = passwordText.getText().equals(
        ...
        if (accountExists)
            updateMessage(Messages.account_exists_errorMessage, ERROR);

        else if (isUsernameEmpty && wasUsernameValid)
            updateMessage(
                Messages.CreateXMPPAccountWizardPage_error_enter_username,
                ERROR);
            ...
            updateMessage(
                Messages.CreateXMPPAccountWizardPage_error_password_no_match,
                ERROR);

        else if (isServerEmpty && wasServerValid)
            updateMessage(
                Messages.CreateXMPPAccountWizardPage_error_enter_server, ERROR);
            ...
            updateMessage(DESCRIPTION, NONE);
