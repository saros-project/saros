...
public class ComponentFinder {
    ...
    public static void main(String... args) throws Exception {
        ...
        String className = ComponentFinder.class.getName().replace(".", "/")
            .concat(".class");

        URL location = ClassLoader.getSystemClassLoader()
            .getResource(className);

        File directory = new File(location.toURI());
        File file = new File(className);

        startDirectory = new File(directory.getPath().substring(0,
            directory.getPath().length() - file.getPath().length()));
