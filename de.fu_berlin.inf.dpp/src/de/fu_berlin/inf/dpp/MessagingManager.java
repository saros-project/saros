...
public class MessagingManager implements PacketListener, MessageListener,
	...
	XMPPMultiChatTransmitter multitrans = null;
	...
	public class MultiChatSession implements SessionProvider, PacketListener,
		...
		public void processPacket(Packet packet) {
			...
			Display.getDefault().syncExec(new Runnable() {
				public void run() {
					openWindow();
					String from = message.getFrom().replace(XMPPMultiChatTransmitter.Room+"/", "").replace("/Smack", "");
	...
	public MessagingManager() {
		...
		this.multitrans = new XMPPMultiChatTransmitter();
	}
	...
	public void processPacket(Packet packet) {
		...
		if (message.getFrom().contains(XMPPMultiChatTransmitter.Room)) {
