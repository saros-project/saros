...
public class OctTreeQuantizer implements Quantizer {
    ...
    private static final SmackLogger LOGGER = SmackLogger
        .getLogger(OctTreeQuantizer.class);
    ...
    class OctTreeNode {
        ...
        public void list(PrintStream s, int level) {
            ...
            if (count == 0)
                ...
                LOGGER.debug(indentStr + index + ": count=" + count + " red="
    ...
    private int buildColorTable(OctTreeNode node, int[] table, int index) {
        ...
        if (node.isLeaf) {
            ...
            table[index] = 0xff000000 | ((node.totalRed / count) << 16)
                | ((node.totalGreen / count) << 8) | node.totalBlue / count;
            node.index = index++;
        } else {
            for (int i = 0; i < 8; i++) {
                if (node.leaf[i] != null) {
