 ...
 * Operation about to be undone and calls instead this.undo(). Warning: After
...
public class UndoManager extends AbstractActivityProvider implements Disposable {
	...
	protected InclusionTransformation transformation =
		new GOTOInclusionTransformation();
	...
	protected ISharedEditorListener sharedEditorListener =
		new AbstractSharedEditorListener() {
			...
			public void activeEditorChanged(User user, SPath newActiveEditor) {
				...
				if (!user.isLocal()
						|| ObjectUtils.equals(currentActiveEditor,
							newActiveEditor))
					return;
	...
	protected IActivityReceiver activityDataObjectReceiver =
		new AbstractActivityReceiver() {
			...
			/**
			 * Updates the current local operation and adds remote operations to
			 * the undo history.
			 */
			@Override
			public void receive(TextEditActivity textEditActivityDataObject) {
				...
				/*
				 * When performing an undo/redo there are fired
				 * activityDataObjects which are expected and have to be ignored
				 * when coming back.
				 */
				...
				if (!local(textEditActivityDataObject)) {
					...
					if (currentLocalAtomicOperation != null) {
						currentLocalAtomicOperation =
							transformation.transform(
								currentLocalAtomicOperation, operation,
								Boolean.FALSE);
					}
	...
	public IOperationHistoryListener historyListener =
		new IOperationHistoryListener() {
			...
			public void historyNotification(OperationHistoryEvent event) {
				...
				/*
				 * OPERATION_ADDED is triggered when Eclipse adds a new
				 * operation to its history. We do the same on this event.
				 */
				...
				/*
				 * OPERATION_CHANGED is triggered when Eclipse changes the
				 * operation that is recently added to its history. For example:
				 * Eclipse adds Insert(3,"A") to its operation and later on the
				 * user enters B at position 4. If Eclipse decides not to add a
				 * new Undo step it changes the most recent operation in history
				 * to Insert(3, "AB")
				 */
	...
	protected Operation calcUndoOperation(SPath editor) {
		...
		for (EditorHistoryEntry entry : undoHistory
			...
			undoOperation =
				transformation
					.transform(undoOperation, operation, Boolean.TRUE);
	...
	protected Operation calcRedoOperation(SPath editor) {
		...
		for (EditorHistoryEntry entry : undoHistory
			...
			redoOperation =
				transformation.transform(redoOperation, entry.getOperation(),
					Boolean.TRUE);
		}
	...
	protected void fireActivity(TextEditActivity activity) {
		...
		List<ITextOperation> textOps =
			activity.toOperation().getTextOperations();
	...
	protected void updateCurrentLocalAtomicOperation(Operation newMRO) {
		if (currentLocalCompositeOperation == null) {
			...
			currentLocalCompositeOperation =
				new SplitOperation(currentLocalCompositeOperation,
					currentLocalAtomicOperation);
