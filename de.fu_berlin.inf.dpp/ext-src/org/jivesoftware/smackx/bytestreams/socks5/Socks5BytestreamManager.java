...
/*
 * Modified version including my (Stefan Rossbach) bug fix:
 * http://issues.igniterealtime.org/browse/SMACK-395
 */
public final class Socks5BytestreamManager implements BytestreamManager {
     ...
     */
    private List<String> ignoredBytestreamRequests = Collections
        .synchronizedList(new LinkedList<String>());
    ...
    public Socks5BytestreamSession establishSession(String targetJID,
        ...
        try {
            ...
            Packet response = SyncPacketSend.getReply(this.connection,
                initiation, getTargetResponseTimeout());

            // extract used stream host from response
            StreamHostUsed streamHostUsed = ((Bytestream) response)
                .getUsedHost();
            StreamHost usedStreamHost = initiation.getStreamHost(streamHostUsed
                .getJID());
            ...
            // build SOCKS5 client
            Socks5Client socks5Client = new Socks5ClientForInitiator(
                usedStreamHost, digest, this.connection, sessionID, targetJID);
            ...
            return new Socks5BytestreamSession(socket, usedStreamHost.getJID()
                .equals(this.connection.getUser()));
    ...
    private boolean supportsSocks5(String targetJID) throws XMPPException {
        ServiceDiscoveryManager serviceDiscoveryManager = ServiceDiscoveryManager
            .getInstanceFor(this.connection);
        DiscoverInfo discoverInfo = serviceDiscoveryManager
            ...
            if (addresses.size() >= 1) {
                ...
                for (String address : addresses) {
                    StreamHost streamHost = new StreamHost(
                        this.connection.getUser(), address);
