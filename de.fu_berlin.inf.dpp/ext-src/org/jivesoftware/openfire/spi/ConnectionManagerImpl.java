...
public class ConnectionManagerImpl extends BasicModule implements ConnectionManager, CertificateEventListener {
    ...
    private synchronized void startListeners() {
		 ...
		 * XMPPServer.getInstance().getPluginManager(); if
		 * (!pluginManager.isExecuted()) {
		 * pluginManager.addPluginManagerListener(new PluginManagerListener() {
		 * public void pluginsMonitored() { // Stop listening for plugin events
		 * XMPPServer
		 * .getInstance().getPluginManager().removePluginManagerListener(this);
		 * // Start listeners startListeners(); } }); return; }
		 */
