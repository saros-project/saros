...
public abstract class StfTestCase {
    ...
    /**
     * Restores Saros session with empty project with given project name if non
     * existant
     *
     * @param projectName
     *            name of the project
     ...
     *            a tester e.g. ALICE
     * @param testers
     *            additional testers e.g. BOB,CARL,DAVE
     * @throws Exception
     *             if a (internal) failure occur
     */
    public static void restoreSessionIfNecessary(String projectName,
        AbstractTester tester, AbstractTester... testers) throws Exception {

        if (!isSession() || !areConnected(tester, testers)
            ...
            Util.buildSessionConcurrently(projectName,
                TypeOfCreateProject.NEW_PROJECT, tester, testers);
        }
        for (AbstractTester othertester : testers) {
            othertester.superBot().views().packageExplorerView()
                .waitUntilResourceIsShared(projectName);
        }
    }
    ...
    public static boolean areConnected(AbstractTester tester,
        AbstractTester... testers) throws Exception {
        boolean areConnected = tester.superBot().views().sarosView()
            .isConnected();
        for (AbstractTester othertester : testers) {
            if (othertester != null) {
                areConnected = areConnected
                    && othertester.superBot().views().sarosView().isConnected();
            }
        }
        return areConnected;
    }
     ...
     *             if a (internal) failure occur
     */
    public static boolean isResourceSharedForAllTesters(String resourcePath,
        AbstractTester tester, AbstractTester... testers) throws Exception {
        boolean isResourceShared = tester.superBot().views()
            .packageExplorerView().isResourceShared(resourcePath);
        for (AbstractTester othertester : testers) {
            if (othertester != null) {
                isResourceShared = isResourceShared
                    && othertester.superBot().views().packageExplorerView()
    ...
    /**
     * Checks if the test is run by itself or is part of a TestSuite. If it is
     * part of a TestSuite, the array allClassNames will contain more than one
     * element.
     *
     */
