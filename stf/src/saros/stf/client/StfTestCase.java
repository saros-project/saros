...
public abstract class StfTestCase {
  ...
  public static ArrayList<String> allClassNames = new ArrayList<String>();
  ...
  public final TestWatcher watcher =
      new TestWatcher() {
        ...
        public void failed(Throwable e, Description description) {
          if (!(e instanceof AssumptionViolatedException)) {
            ...
            this.captureScreenshot(
                (description.getClassName() + "_" + description.getMethodName()).replace('.', '_'));
        ...
        public void starting(Description description) {
          ...
          if (!allClassNames.contains(description.getClassName())) {
            allClassNames.add(description.getClassName());
          }
        }
        ...
        private void captureScreenshot(String name) {
          ...
          while (var3.hasNext()) {
            ...
            try {
              ...
              StfTestCase.LOGGER.log(
                  Level.WARNING,
                  "could capture a screenshot at remote bot of tester " + tester,
                  var5);
            }
          }
        }
   ...
   *
   * <ol>
   *   <li>reset all contact names by calling for the first test case of the category {@linkplain
   *       #initTesters(AbstractTester, AbstractTester...)}
   ...
   * @throws Exception
   */
  public static void selectFirst(AbstractTester tester, AbstractTester... testers)
      throws Exception {
   ...
   *
   * <ol>
   *   <li>reset all contact names by calling for the test cases of the category, which are not the
   *       first case {@linkplain #initTesters(AbstractTester, AbstractTester...)}
   *   <li>{@linkplain #setUpWorkbench()}
   *   <li>{@linkplain #setUpSaros()}
   *   <li>{@linkplain #reconnectAllTesters()}
   * </ol>
  ...
  /**
   * Tries to reset Saros to a stable state for the given tester(s). It does that be performing the
   * following actions (for the last test case of the category)
   *
   * <ol>
   *   <li>reset all contact names by calling {@linkplain #resetNicknames(AbstractTester)}
  ...
  public static boolean isSession() throws Exception {
    for (AbstractTester tester : currentTesters) {
      ...
      if (tester.superBot().views().sarosView().isInSession() == false) return false;
    }
    return true;
  }
  ...
  /**
   * Restores Saros session with empty project with given project name if non existant
   *
   * @param projectName name of the project
   * @param tester a tester e.g. ALICE
   * @param testers additional testers e.g. BOB,CARL,DAVE
   * @throws Exception if a (internal) failure occur
   */
  public static void restoreSessionIfNecessary(
      String projectName, AbstractTester tester, AbstractTester... testers) throws Exception {

    if (!isSession()
        || !areConnected(tester, testers)
        || !isResourceSharedForAllTesters(projectName, tester, testers)) {
      disconnectAllActiveTesters();
      clearWorkspaces();
      reconnectAllTesters();
      tester.superBot().internal().createProject(projectName);
      Util.buildSessionConcurrently(projectName, TypeOfCreateProject.NEW_PROJECT, tester, testers);
    }
    for (AbstractTester othertester : testers) {
      othertester.superBot().views().packageExplorerView().waitUntilResourceIsShared(projectName);
    }
  }

  /**
   * Checks if all given users are connected
   *
   * @param tester a tester e.g. ALICE
   * @param testers additional testers e.g. BOB,CARL,DAVE
   * @throws Exception if a (internal) failure occur
   */
  public static boolean areConnected(AbstractTester tester, AbstractTester... testers)
      throws Exception {
    boolean areConnected = tester.superBot().views().sarosView().isConnected();
    for (AbstractTester othertester : testers) {
      if (othertester != null) {
        areConnected = areConnected && othertester.superBot().views().sarosView().isConnected();
      }
    }
    return areConnected;
  }
  ...
  /**
   * Checks if a resource is Shared for all testers
   *
   * @param projectName name of the project
   * @param tester a tester e.g. ALICE
   * @param testers additional testers e.g. BOB,CARL,DAVE
   * @throws Exception if a (internal) failure occur
   */
  public static boolean isResourceSharedForAllTesters(
      String resourcePath, AbstractTester tester, AbstractTester... testers) throws Exception {
    boolean isResourceShared =
        tester.superBot().views().packageExplorerView().isResourceShared(resourcePath);
    for (AbstractTester othertester : testers) {
      if (othertester != null) {
        isResourceShared =
            isResourceShared
                && othertester
                    ...
                    .views()
                    .packageExplorerView()
                    .isResourceShared(resourcePath);
      }
    }
    return isResourceShared;
  }

  public static ArrayList<String> getAllClassNamesList() {
    return allClassNames;
  }

  /**
   * Checks if the test is run by itself or is part of a TestSuite. If it is part of a TestSuite,
   * the array allClassNames will contain more than one element.
   */
  public static boolean checkIfTestRunInTestSuite() {
    if (getAllClassNamesList().size() > 1) {
      return true;
    }
    return false;
  }
