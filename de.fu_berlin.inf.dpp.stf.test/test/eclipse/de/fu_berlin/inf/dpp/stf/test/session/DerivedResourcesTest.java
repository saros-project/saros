...
public class DerivedResourcesTest extends StfTestCase {
    ...
    public void testDerivedResourcesMustNotMarkedAsSharedInAFullSharedProject()
        ...
        Util.setUpSessionWithJavaProjectAndClass("Foo", "bar", "Foo", ALICE,
            BOB);

        BOB.superBot().views().packageExplorerView()
            .waitUntilResourceIsShared("Foo/src/bar/Foo.java");

        ALICE.remoteBot().view(Constants.VIEW_PACKAGE_EXPLORER).show();

        ALICE.remoteBot().view(Constants.VIEW_PACKAGE_EXPLORER).bot().tree()
            .selectTreeItemWithRegex("Foo.*").clickContextMenu("Build Project");
        ...
        ALICE.superBot().views().packageExplorerView()
            .waitUntilFileExists("Foo/bin/bar/Foo.class");

        assertFalse(
            "compiled class file is marked as shared",
            ALICE.superBot().views().packageExplorerView()
                .isResourceShared("Foo/bin/bar/Foo.class"));
    }
}
