...
public class SimpleNoteCompositeDemo extends DescriptiveDemo {
	public SimpleNoteCompositeDemo(DemoContainer demoContainer, String title) {
		super(demoContainer, title);
	}

	@Override
	public String getDescription() {
		return "Different " + NoteComposite.class.getSimpleName()
				+ "s with various system icons and colors";
	}

	protected SimpleNoteComposite createNoteComposite(Composite parent,
			int iconId, int fgColorId, int bgColorId, String text) {
		SimpleNoteComposite noteComposite = new SimpleNoteComposite(parent,
				SWT.BORDER);
		...
		if (iconId != SWT.DEFAULT)
			noteComposite.setImage(iconId);
	...
	public void createContent(Composite parent) {
		...
		int[] fgColorIds = new int[] { SWT.DEFAULT, SWT.COLOR_BLACK,
				SWT.COLOR_WHITE, SWT.COLOR_WHITE, SWT.COLOR_WHITE,
				SWT.COLOR_BLACK, SWT.COLOR_BLACK, SWT.COLOR_WHITE };
		int[] bgColorIds = new int[] { SWT.DEFAULT,
				SWT.COLOR_WIDGET_BACKGROUND, SWT.COLOR_WIDGET_BORDER,
				SWT.COLOR_WIDGET_DARK_SHADOW, SWT.COLOR_WIDGET_FOREGROUND,
				SWT.COLOR_WIDGET_HIGHLIGHT_SHADOW,
				SWT.COLOR_WIDGET_LIGHT_SHADOW, SWT.COLOR_WIDGET_NORMAL_SHADOW };
		String[] texts = new String[] { "SWT.DEFAULT\nSWT.DEFAULT",
		...
		for (int i = 0; i < iconIds.length; i++) {
			...
			String text = "I'm a " + NoteComposite.class.getSimpleName()
					+ " with the following options:\n" + texts[i] + ".";
			SimpleNoteComposite noteComposite = createNoteComposite(parent,
					iconIds[i], fgColorIds[i], bgColorIds[i], text);
			noteComposite
