...
import saros.editor.annotations.RemoteCursorStrategy;
import saros.editor.annotations.SarosAnnotation;
import saros.editor.annotations.SelectionFillUpAnnotation;
import saros.editor.annotations.SelectionFillUpStrategy;
import saros.editor.internal.AnnotationModelHelper;
import saros.editor.internal.ContributionAnnotationManager;
import saros.editor.internal.CustomAnnotationManager;
import saros.editor.internal.EditorAPI;
import saros.editor.internal.LocationAnnotationManager;
import saros.editor.internal.SafePartListener2;
...
public class EditorManager implements IEditorManager {
   ...
   */
  void generateEditorActivated(saros.filesystem.IFile file) {
  ...
  void generateViewport(IEditorPart part, LineRange viewport) {
    ...
    if (wrappedFile.equals(locallyActiveEditor)) this.localViewport = viewport;
   ...
   *
   * @param file the file whose editor to close
  ...
  void partActivated(IEditorPart editorPart) {
    ...
    generateEditorActivated(editorFile);
  ...
  /**
   * Checks whether given file is currently opened.
   *
  ...
  /**
   * Save file denoted by the given file if necessary according to isDirty(IPath) and call
   * saveText(IPath) if necessary in the SWT thread.
   *
   * @blocking This method returns after the file has been saved in the SWT Thread.
   */
  public void saveLazy(final saros.filesystem.IFile file) {
   ...
   */
  public Set<saros.filesystem.IFile> getRemoteOpenEditors(User user) {
