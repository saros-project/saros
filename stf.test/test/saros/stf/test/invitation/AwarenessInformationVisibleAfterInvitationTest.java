...
public class AwarenessInformationVisibleAfterInvitationTest
    extends StfTestCase {
     ...
     *
     * <p>
     * 1. Alice creates local Project with file1 and opens file1
     *
     * <p>
     * 2. Alice invites Bob
     *
    ...
    public void awarenessInformationVisibleNewSessionTest() throws Exception {
        ...
        assertEquals("Bob sees the wrong file as open by Alice or none at all.",
     ...
     *
     * <p>
     * 1. Alice creates local Project with file1 and file2 and opens file1
     *
     * <p>
     * 2. Alice invites Bob
     *
     ...
     *
     * <p>
     * 4. Alice adds Carl to the session
     *
     * <p>
     * 5. Carl checks if he can see which files Alice and Bob have opened
     */
    @Test
    public void awarenessInformationVisableAddToSessionTest() throws Exception {
        ...
        BOB.superBot().views().packageExplorerView()
            .selectFile(Constants.PROJECT1, Constants.FILE2).open();
        ...
        CARL.superBot().views().packageExplorerView().waitUntilResourceIsShared(
            Constants.PROJECT1 + "/" + Constants.FILE2);
        ...
        String expectedOpenProjectAlice = Constants.PROJECT1 + ": "
            + Constants.FILE1;
        String expectedOpenProjectBob = Constants.PROJECT1 + ": "
            + Constants.FILE2;

        String projectOpenAtAlice = CARL.remoteBot().view("Saros").bot().tree()
            .selectTreeItem("Session")
            .getNodeWithRegex(".*" + Pattern.quote(ALICE.getName()) + ".*")
            .getNode(0).getText();
        String projectOpenAtBob = CARL.remoteBot().view("Saros").bot().tree()
            .selectTreeItem("Session")
        ...
        assertEquals(
            ...
            expectedOpenProjectAlice, projectOpenAtAlice);

        assertEquals("Carl sees the wrong file as open by Bob or none at all.",
            expectedOpenProjectBob, projectOpenAtBob);
    }
}
