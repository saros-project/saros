...
public class ShareProjectWizardUITest extends StfTestCase {
    ...
    private void removeSelection(String name) throws Exception {
        ...
        assertTrue("Could not remove selection ",
            ArrayUtils
                .indexOf(
                    ALICE.remoteBot().shell(SHELL_SHARE_PROJECT).bot()
                        .comboBox(0).items(),
                    name) == ArrayUtils.INDEX_NOT_FOUND);
    }
    ...
    private void storeSelection(String name) throws Exception {
        ...
        if (ALICE.remoteBot()
            .isShellOpen(ResourceSelectionComposite_overwrite_dialog_title)) {
            ALICE.remoteBot()
                .shell(ResourceSelectionComposite_overwrite_dialog_title).bot()
                .button(YES).click();
        }

        assertTrue("Could not store selection",
            ArrayUtils
                .indexOf(
                    ALICE.remoteBot().shell(SHELL_SHARE_PROJECT).bot()
                        .comboBox(0).items(),
