...
public class ConcurrentEditingInsert100CharactersTest extends StfTestCase {
    ...
    public static void selectTesters() throws Exception {
        Assume.assumeTrue(
            checkIfShare3UsersConcurrentlyandConcurrentEditingSucceeded());
    ...
    public void testInsertCharactersOnTheSameLine() throws Exception {
        ...
        ALICE.superBot().internal().createFile("foo", "readme.txt",
        ...
        aliceEditTaskThread = createTestThread(
            new TypeTask(ALICE, "readme.txt", 'A', 'Z'));

        aliceEditTaskThread.start();

        bobEditTaskThread = createTestThread(
            new TypeTask(BOB, "readme.txt", 'a', 'z'));

        bobEditTaskThread.start();

        carlEditTaskThread = createTestThread(
            new TypeTask(CARL, "readme.txt", '0', '9'));
