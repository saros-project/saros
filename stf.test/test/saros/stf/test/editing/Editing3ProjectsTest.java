...
public class Editing3ProjectsTest extends StfTestCase {
    ...
    public void testEditing3Projects() throws Exception {
        ALICE.superBot().internal().createFile("Foo1_Saros",
            "src/bar/HelloAlice.java", "HelloAlice");
        BOB.superBot().views().packageExplorerView()
            .waitUntilResourceIsShared("Foo1_Saros/src/bar/HelloAlice.java");
        ALICE.superBot().internal().createJavaProject("Foo2_Saros");
        ALICE.superBot().internal().createFile("Foo2_Saros",
            "src/bar/HelloBob.java", "HelloBob");
        Thread.sleep(1000);
        Util.addProjectToSessionSequentially("Foo2_Saros",
        ...
        ALICE.superBot().internal().createFile("Foo3_Saros",
            "src/bar/HelloCarl.java", "HelloCarl");

        Util.addProjectToSessionSequentially("Foo3_Saros",
            TypeOfCreateProject.NEW_PROJECT, ALICE, BOB);

        BOB.superBot().views().packageExplorerView()
            .waitUntilResourceIsShared("Foo3_Saros");

        BOB.superBot().views().packageExplorerView()
            .selectClass("Foo1_Saros", "bar", "HelloAlice").open();

        BOB.remoteBot().editor("HelloAlice.java").waitUntilIsActive();

        BOB.superBot().views().packageExplorerView()
            .selectClass("Foo2_Saros", "bar", "HelloBob").open();

        BOB.remoteBot().editor("HelloBob.java").waitUntilIsActive();

        BOB.superBot().views().packageExplorerView()
            .selectClass("Foo3_Saros", "bar", "HelloCarl").open();
        ...
        assertEquals(ALICE.remoteBot().editor("HelloAlice.java").getText(),
            BOB.remoteBot().editor("HelloAlice.java").getText());
        assertEquals(ALICE.remoteBot().editor("HelloBob.java").getText(),
            BOB.remoteBot().editor("HelloBob.java").getText());
        assertEquals(ALICE.remoteBot().editor("HelloCarl.java").getText(),
