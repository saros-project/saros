...
public class ObjectCreationTest extends StfTestCase {
    ...
    public static void selectTesters() throws Exception {
        ...
        select(ALICE, BOB);
        Util.setUpSessionWithProjectAndFile("foo", "readme.txt", fileContent,
            ALICE, BOB);
        for (AbstractTester tester : getCurrentTesters()) {
            if (!tester.remoteBot()
                .isViewOpen(Constants.VIEW_SAROS_WHITEBOARD)) {
                tester.superBot().menuBar().window().showViewWithName(
                    Constants.NODE_SAROS, Constants.VIEW_SAROS_WHITEBOARD);
            }
    ...
    private void checkObjectFromActiveTool(Tool activeTool) throws Exception {
        ...
        ALICE.superBot().views().sarosWhiteboardView()
            .activateTool(activeTool.getLabel());
        ALICE.superBot().views().sarosWhiteboardView()
            .createObjectFromActiveTool(x, y, width, height);
        Thread.sleep(2000);

        // obtain object figure from each bob and alice
        IWhiteboardFigure rectAlice = ALICE.superBot().views()
            .sarosWhiteboardView().getFigureAt(clickX, clickY);
        IWhiteboardFigure rectBob = BOB.superBot().views().sarosWhiteboardView()
            .getFigureAt(clickX, clickY);
        // check if the figure created on Bob's whiteboard equals the figure of
        // Alice
        checkWhiteboardfigure(rectAlice, rectBob);
    }

    private void checkWhiteboardfigure(IWhiteboardFigure figure1,
        IWhiteboardFigure figure2) throws Exception {
        ...
        org.junit.Assert.assertEquals(figure1.getLocation(),
            figure2.getLocation());
        org.junit.Assert.assertEquals(figure1.getBackgroundColor(),
            figure2.getBackgroundColor());
        org.junit.Assert.assertEquals(figure1.getForegroundColor(),
            figure2.getForegroundColor());
    }
}
