 ...
 */
public class EstablishSessionWithDifferentTransportModesTest
    extends StfTestCase {
    ...
    private void simulateSession() throws Exception {
        ...
        assertEquals(BOB.remoteBot().editor("bar.txt").getText(),
            ALICE.remoteBot().editor("bar.txt").getText());
    }
    ...
    private static void activateTransportMode(String transportMode)
        throws Exception {
        ...
        for (AbstractTester tester : getCurrentTesters()) {
            ...
            shell.activate();
            shell.bot().tree().expandNode(NODE_SAROS)
                .select(NODE_SAROS_NETWORK);
            ...
            if (checkBoxText != null) {
                shell.bot().checkBoxInGroup(checkBoxText,
                    PREF_NODE_SAROS_NETWORK_TRANSPORT_MODE_GROUP).select();
            }
