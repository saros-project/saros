...
public class CreatingNewFileTest extends StfTestCase {
  ...
  public void testCreatingNewFileTest() throws Exception {
    ...
    CARL.superBot().views().packageExplorerView().waitUntilResourceIsShared("Foo1_Saros");
    BOB.superBot().views().packageExplorerView().waitUntilResourceIsShared("Foo1_Saros");

    ALICE.superBot().internal().createFile("Foo1_Saros", "src/readme.txt", "this is a test case");
    ...
    ALICE.superBot().views().sarosView().selectUser(CARL.getJID()).restrictToReadOnlyAccess();

    ALICE.superBot().views().sarosView().selectUser(CARL.getJID()).waitUntilHasReadOnlyAccess();
    ...
    ALICE.superBot().views().sarosView().selectUser(CARL.getJID()).followParticipant();
    ...
    CARL.superBot().internal().createFile("Foo1_Saros", "bar/readme.txt", "not visible");

    CARL.superBot()
        .views()
        .packageExplorerView()
        .selectFile("Foo1_Saros", "bar", "readme.txt")
        .open();
    ...
    assertFalse("Carls editor must not be opened", ALICE.remoteBot().isEditorOpen("readme.txt"));
    ...
    assertFalse(
        ...
        BOB.superBot().views().packageExplorerView().isResourceShared("Foo1_Saros/bar/readme.txt"));

    assertEquals(
        "Alice had changed a file during read only access while typing",
        CARL.remoteBot().editor("readme.txt").getText(),
        "not visible");

    try {
      CARL.superBot().views().sarosView().waitUntilIsInconsistencyDetected();
