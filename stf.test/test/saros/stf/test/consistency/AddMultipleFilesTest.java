...
public class AddMultipleFilesTest extends StfTestCase {
  ...
  public void testAddMultipleFilesSimultaneouslyTest() throws Exception {
    ...
    ALICE.superBot().internal().createJavaClass("Foo1_Saros", "my.pkg", "MyClass");
    BOB.superBot()
        .views()
        .packageExplorerView()
          ...
          public void run() throws Exception {
            for (int i = 0; i < 10; i++) {
              ...
              ALICE
                  .superBot()
                  .internal()
                  .createFile("Foo1_Saros", "src/my/pkg/bigfile" + i, 10 * 1024 * 1024, true);
            }
          }
        };
        ...
        new EclipseTestThread.Runnable() {
          ...
          public void run() throws Exception {
            for (int i = 0; i < 1000; i++) {
              ...
              BOB.superBot()
                  .internal()
                  .createFile("Foo1_Saros", "src/my/pkg/smallfile" + i, 100 * 1024, true);
            }
          }
        };
    ...
    for (int i = 0; i < 1000; i++) {
      assertTrue(
          "file " + "Foo1_Saros/src/my/pkg/smallfile" + i + " does not exist on ALICEs side",
          ALICE.superBot().internal().existsResource("Foo1_Saros/src/my/pkg/smallfile" + i));

      assertTrue(
          "file " + "Foo1_Saros/src/my/pkg/smallfile" + i + " does not exist on BOBs side",
          BOB.superBot().internal().existsResource("Foo1_Saros/src/my/pkg/smallfile" + i));
    }
  }
}
