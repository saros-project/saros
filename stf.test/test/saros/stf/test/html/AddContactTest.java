...
public class AddContactTest extends StfHtmlTestCase {
    ...
    public void addBobAsContact() throws Exception {
        // Precondition
        List<String> aliceContactList = ALICE.htmlBot()
            .getContactList(MAIN_VIEW);
        ...
        assertFalse("Bob still has Alice as contact",
            bobContactList.contains(ALICE.getBaseJid()));
        ...
        // add bob as contact
        ALICE.htmlBot().view(ADD_CONTACT).inputField("jid")
            .enter(BOB.getBaseJid());
        ALICE.htmlBot().view(ADD_CONTACT).button("add-contact").click();
        assertTrue("Main View did not open",
