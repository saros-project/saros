 ...
 * methods that the page calls inside Javascript. The renderers transfer
 * application state from Java to the page.
 * 
 * @JTourBusStop 2, Extending the HTML GUI, Creating a Java abstraction for a
 *               page:
 * 
 *               Each page in Saros has a corresponding implementation of this
 *               interface. There is one for the Saros main view and one for
 *               each dialog. So if you add a new .html file, you should add a
...
public interface IBrowserPage {
    /**
     * Returns the relative path to the corresponding HTML file of this
     * <code>BowserPage</code>.
     * <p/>
