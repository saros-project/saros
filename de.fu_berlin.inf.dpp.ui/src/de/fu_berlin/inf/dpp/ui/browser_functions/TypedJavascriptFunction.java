...
/**
 * Offers type-safety at the boundary between the JavaScript and Java worlds by
 * overwriting the generic {@link #function(Object[])} method.
 * <p>
 * Subclasses need to provide the desired browser function as a standard Java
 * method annotated with {@link BrowserFunction}. This method will be called
 * with the correctly typed and, for complex types, deserialized input from the
...
class TypedJavascriptFunction extends SelfRegisteringJavascriptFunction {
     ...
     *         annotation
     */
    protected static Method getCallable(
        Class<? extends TypedJavascriptFunction> clazz) {
        ...
        if (callable == null) {
            throw new IllegalArgumentException("class " + clazz.getName()
                + " has no annotated callable method");
        }
    ...
    /**
     * Creates a new {@link TypedJavascriptFunction} with a name following the
     * {@linkplain #JS_NAME_CONVENTION JavaScript naming convention}. This
     ...
     *             be converted to the specified Java type
     */
    private List<Object> getTypedArguments(Object[] arguments) {
        ...
        if (arguments.length != argTypes.length) {
            LOG.error(getName()
                ...
                + arguments.length + ", expected: " + argTypes.length);
    ...
    /**
     * Run asynchronous callables through ThreadUtils (their return value, if
     * any, is disregarded)
     */
    private void invokeAsync(final List<Object> typedArguments) {
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                try {
                    callable.invoke(TypedJavascriptFunction.this,
                        typedArguments.toArray());
                } catch (Exception e) {
    ...
    private Object invokeSync(final List<Object> typedArguments) {
        ...
        Class<?>[] allowedTypes = new Class<?>[] { Boolean.class, String.class,
            Integer.class, Long.class, Float.class, Double.class };

        if (Arrays.asList(allowedTypes).contains(returnType))
    ...
    private void logInvocationError(Exception e, List<Object> typedArguments) {
        ...
        if (clazz == InvocationTargetException.class) {
            LOG.error(name + " threw an exception, args: " + typedArguments,
                e.getCause());
            ...
            LOG.error(name + " was not called properly, args: "
                + typedArguments, e);
        }
    }
}
