...
public class EnterProjectNamePageTest {
    ...
    public static void setupBeforeClass() {
        page = EasyMock.createMockBuilder(EnterProjectNamePage.class)
            .addMockedMethod("projectNameIsUnique").createMock();

        // EasyMock does not support varargs, therefore we just assume two
        // String parameters
        page.projectNameIsUnique(EasyMock.isA(String.class),
            EasyMock.isA(String.class));
        EasyMock.expectLastCall().andStubAnswer(new IAnswer<Boolean>() {
            @Override
