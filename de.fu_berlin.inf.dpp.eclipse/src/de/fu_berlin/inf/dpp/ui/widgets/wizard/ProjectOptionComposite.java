...
public class ProjectOptionComposite extends Composite {
    ...
    /**
     * Returns the remote project ID that is associated with the project
     * represented by this composite.
     * 
    ...
    /**
     * Returns whether the remote project should merged into an already existing
     * one.
     * 
     * @return <code>true</code> if the remote project should be merged,
     ...
     * 
     * @param useExistingProject
     *            <code>true</code>, if the local project should point to an
     *            existing local project, <code>false</code> otherwise
     * @param name
    ...
    private void createNewProjectGroup() {
        ...
        newProjectRadioButton
            .setText(Messages.EnterProjectNamePage_create_new_project);
        newProjectRadioButton.setSelection(true);
        ...
        newProjectNameLabel.setLayoutData(new GridData(SWT.BEGINNING,
        ...
        newProjectNameText.addModifyListener(new ModifyListener() {
            ...
            public void modifyText(ModifyEvent e) {
                for (ProjectOptionListener listener : listeners) {
                    listener.projectNameChanged(new ProjectNameChangedEvent(
                        getRemoteProjectID(), getProjectName()));
                }
            }
        });
    ...
    private void createUpdateProjectGroup() {
        ...
        existingProjectRadioButton
            .setText(Messages.EnterProjectNamePage_use_existing_project);
        existingProjectRadioButton.setSelection(false);
        ...
        updateProjectNameLabel.setLayoutData(new GridData(SWT.BEGINNING,
            SWT.BEGINNING, false, false));

        /* Text box */
        existingProjectNameText = new Text(this, SWT.BORDER);
        existingProjectNameText.setLayoutData(new GridData(SWT.FILL,
            SWT.BEGINNING, true, false));
        existingProjectNameText.setEnabled(false);
        existingProjectNameText.addModifyListener(new ModifyListener() {
            @Override
            public void modifyText(ModifyEvent e) {
                for (ProjectOptionListener listener : listeners)
                    listener.projectNameChanged(new ProjectNameChangedEvent(
        ...
        browseProjectsButton.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                String projectName = getProjectDialog(Messages.EnterProjectNamePage_select_project_for_update);
                if (projectName != null)
                    existingProjectNameText.setText(projectName);
            }
        });
    }
