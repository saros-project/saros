...
public class EclipsePreferenceInitializer extends AbstractPreferenceInitializer {
    ...
    private static void setPreferences(final PreferenceAccessAdapter adapter) {
        ...
        PreferenceInitializer.initialize(new CorePreferenceStoreAccessAdapter(
            adapter));

        adapter.setDefault(EclipsePreferenceConstants.GATEWAYCHECKPERFORMED,
            false);

        // Advanced Preferences

        adapter.setDefault(EclipsePreferenceConstants.AUTO_STOP_EMPTY_SESSION,
            MessageDialogWithToggle.PROMPT);

        adapter.setDefault(
            EclipsePreferenceConstants.ENABLE_BALLOON_NOTIFICATION, true);
        ...
        adapter.setDefault(EclipsePreferenceConstants.FEEDBACK_SURVEY_DISABLED,
            FeedbackManager.FEEDBACK_ENABLED);
        adapter.setDefault(EclipsePreferenceConstants.FEEDBACK_SURVEY_INTERVAL,
        ...
        adapter.setDefault(
            EclipsePreferenceConstants.USE_IRC_STYLE_CHAT_LAYOUT, false);
        adapter.setDefault(EclipsePreferenceConstants.CUSTOM_MUC_SERVICE, "");
        adapter.setDefault(EclipsePreferenceConstants.FORCE_CUSTOM_MUC_SERVICE,
            true);
         ...
         */
        adapter.setDefault(
            EclipsePreferenceConstants.SAROSVIEW_SASH_WEIGHT_LEFT, 1);
        adapter.setDefault(
            EclipsePreferenceConstants.SAROSVIEW_SASH_WEIGHT_RIGHT, 1);
    ...
    private static class OSGiPreferencesAccessAdapter implements
        PreferenceAccessAdapter {
    ...
    private static class IPreferenceStoreAccessAdapter implements
        PreferenceAccessAdapter {
        private final IPreferenceStore preferenceStore;

        private IPreferenceStoreAccessAdapter(
    ...
    private static class CorePreferenceStoreAccessAdapter implements
        de.fu_berlin.inf.dpp.preferences.IPreferenceStore {

        private final PreferenceAccessAdapter adapter;

        private CorePreferenceStoreAccessAdapter(
            final PreferenceAccessAdapter adapter) {
