...
/**
 * <p>
 * Achieves a list of LayoutElementRecord from the Clipboard. The list should be
 * obtained by the root records of a {@link HierarichalRecordList} and thus not
 * contain any descendant of any other contained element.
 * </p>
 * 
 * <p>
 * For the execution, the records and all their descendants are copied while the
 * position (all concerning layout attributes) are shifted
 * <code>SHIFT_FOR_COPY</code> points to the right bottom to give feedback about
 ...
 * 
 * <p>
 * An undo corresponds to a delete, a redo to an undo-delet. For better reuse a
 * {@link DeleteRecordsCommand} is used as member.
 * </p>
 * 
 * 
 * @author jurke
...
public class PasteRecordCommand extends SXECommand {
    ...
    protected boolean canExecuteSXECommand() {
        if (deleteCommand == null)
            return !clonedTopRecords.isEmpty();
    ...
    /**
     * Short helper that returns that checks whether the passed AttributeRecord
     * <code>attribute</code> is a layout record (contained by the layout
     * records) if so, it will return the layout record els the passed
     * AttributeRecord itself.
     * 
     * 
     * @param attribute
     * @param layout
     *            layout attributes
     * @return the attribute if it's name is not beyond the AttributeRecords of
    ...
    /**
     * Copies the passed LayoutElementRecord, shifts the layout records and
     * copies all other descendants (attributes, child elements and their child
     * nodes) to finally return them all as a single list.
     * 
     * @param toCopy
     * @return a list of copies of the passes record plus copies if its
     *         descendants
     */
