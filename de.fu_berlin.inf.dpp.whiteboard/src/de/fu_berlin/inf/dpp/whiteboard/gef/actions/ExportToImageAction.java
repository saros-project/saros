...
public class ExportToImageAction extends WorkbenchPartAction {
	...
	protected boolean calculateEnabled() {
		...
		return !(viewer == null || viewer.getContents() == null || viewer
				.getContents().getChildren().isEmpty());
	}
	...
	private void export() {
		GraphicalViewer graphicalViewer = (GraphicalViewer) getWorkbenchPart()
				.getAdapter(GraphicalViewer.class);
		LayerManager layerManager = (LayerManager) graphicalViewer
	...
	private void obtainFilenameAndFormat() {
		FileDialog exportDialog = new FileDialog(
	...
	public static void exportImage(int format, String filename,
			IFigure rootFigure) {
		...
		// obtain a painter
		Image diagramImage = new Image(Display.getDefault(), boundingBox.width,
				boundingBox.height);
	...
	private static void paintChildren(Graphics g, IFigure figure) {
		for (Object child : figure.getChildren()) {
			...
			if (child instanceof FreeformLayer
					&& !(child instanceof ConnectionLayer)) {
