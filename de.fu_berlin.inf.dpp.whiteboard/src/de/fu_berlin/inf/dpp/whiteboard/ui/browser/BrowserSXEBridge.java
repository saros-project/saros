...
/**
 * BrowserSXEBridge is the java interface between the SXEController which
 * contains all model data and the browser which contains the whiteboard and
...
public class BrowserSXEBridge {
    ...
    private static Gson gsonOnlyExposed = new GsonBuilder()
    ...
    /**
     * Creates the action executor and initialises the message listener and the
     * browser function
     * 
     * @param sxe
     *            SXEController created in {@link WhiteboardManager}
     * @param browser
     *            the browser created in the view
     */
    ...
    public void init() {
        ...
        browser.asyncRun("setState("
            + gsonOnlyExposed.toJson(sxe.getDocumentRecord().getRoot()) + ");");
    ...
    private final BrowserRunnable browserFunction = new BrowserRunnable() {
        @Override
