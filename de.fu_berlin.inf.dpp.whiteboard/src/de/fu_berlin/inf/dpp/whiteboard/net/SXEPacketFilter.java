 ...
 * a certain session belonging to a selection of message types.
 * 
 * @author jurke
 * 
 */
public class SXEPacketFilter implements PacketFilter {
    ...
    public SXEPacketFilter(SXESession session, String peer,
        SXEMessageType... messageTypes) {
        this.peer = peer;
            ...
            if (extension != null) {
                ...
                if (peer != null)
                    if (!packet.getFrom().equals(peer)) {
                        log.debug("Received SXE packet "
                            + extension.getMessage().getMessageType()
                            + " from wrong peer: " + packet.getFrom());
                        return false;
                    }
                ...
                if (session != null)
                    if (!extension.getMessage().getSession().equals(session)) {
                        log.debug("Received SXE packet "
                            + extension.getMessage().getMessageType()
                            + " from non session member: " + packet.getFrom());
                        return false;
                    }
                ...
                return messageTypes.contains(extension.getMessage()
                    .getMessageType());
            }
