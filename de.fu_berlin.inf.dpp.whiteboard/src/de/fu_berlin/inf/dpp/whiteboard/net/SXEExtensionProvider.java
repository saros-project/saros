...
public class SXEExtensionProvider implements PacketExtensionProvider {
	...
	private static final Logger log = Logger
			.getLogger(SXEExtensionProvider.class);
	 ...
	 */
	protected void fillPairsfromAttributes(RecordDataObject rdo,
			XmlPullParser xpp) {
		for (int i = 0; i < xpp.getAttributeCount(); i++) {
			rdo.putValue(RecordEntry.fromString(xpp.getAttributeName(i)),
	...
	public PacketExtension parseExtension(XmlPullParser xpp) throws Exception {
		...
		if (!xpp.getName().equals(SXEMessage.SXE_TAG)) {
			...
			pe.setMessage(getMessageInfo(xpp));
			eventType = xpp.next();
		}
		...
		while (eventType != XmlPullParser.END_DOCUMENT) {
			...
			if (eventType == XmlPullParser.START_TAG) {
				if (messageType == null) {
					...
					if (messageType != null
							&& messageType != SXEMessageType.RECORDS) {
						eventType = xpp.next();
						continue;
					}
					if (messageType == null)
						log.warn("Could not distinguish message type from "
								+ xpp.getName());
				}

				if (isRecord(xpp.getName())) {
					pe.getMessage().setRecords(getRecords(xpp));
					eventType = xpp.getEventType();
				} else if (xpp.getName().equals("description")) {
					// TODO read description
					if (messageType != SXEMessageType.STATE_OFFER)
						log.warn("Wrong entry 'description' in " + messageType
								+ ". Only allowed in "
								+ SXEMessageType.STATE_OFFER + " message.");
				} else if (xpp.getName().equals("document-begin")) {

					if (messageType != SXEMessageType.STATE)
						log.warn("Wrong entry 'document-begin' in "
								+ messageType + ". Only allowed in "
								+ SXEMessageType.STATE + " message.");
				} else if (xpp.getName().equals("document-end")) {

					if (messageType != SXEMessageType.STATE)
						log.warn("Wrong entry 'document-end' in " + messageType
								+ ". Only allowed in " + SXEMessageType.STATE
								+ " message.");
				} else
					log.warn("Unknown tag: " + xpp.getName()
		...
		pe.getMessage().setMessageType(messageType);
