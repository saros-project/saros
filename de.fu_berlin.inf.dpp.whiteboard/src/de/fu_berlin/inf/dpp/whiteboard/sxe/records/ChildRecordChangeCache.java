...
/**
 * <p>
 * This class offers convenient methods to notify clients (like GUI) about
 * applied changes and conflicts in the SXE document.
 * </p>
 * 
 * <p>
 * Therefore it allows to cache child and attribute changes (order, create or
 * remove) as well as conflicts until the notification takes place clearing the
 * cache.</br>
 * 
 * These changes can only occur when applying records in this package so
 * respective methods are not public.
 * </p>
 * 
 * <p>
 * The listeners are notified after a whole operation or message is applied.
 * </p>
 * 
...
public class ChildRecordChangeCache {
    ...
    /**
     * Informs about child element and attribute changes as well as conflicts of
     * child nodes by passing the causative records.
     * 
     * @author jurke
     * 
    ...
    public static interface ChildRecordChangeListener {
        ...
        public void childRecordConflict(
            Map<NodeRecord, Set<SetRecord>> conflicts);
    ...
    protected Map<NodeRecord, Set<SetRecord>> conflicts = new LinkedHashMap<NodeRecord, Set<SetRecord>>();

    protected SXEController controller;

    /**
     * @param record
     *            the ElementRecord for which to track events
     * @param controller
     *            the controller that handles record applying
     */
    ...
    /**
     * Add the record that caused a creation, removal or change of a child
     * element
     * 
     * @param record
     */
     ...
     * 
     * @param child
     *            the child that contains the resulting (reverted) state
     * @param previousState
     *            the SetRecord that used to be valid before the conflict
     * @param remote
     *            the SetRecord causing the conflict
     */
    void addChildRecordConflict(NodeRecord child, SetRecord previousState,
        SetRecord remote) {
    ...
    /**
     * Notifies all listeners about change and conflict events and clears the
     * cache.
     */
    public void notifyListeners() {
        ...
        for (ChildRecordChangeListener l : listeners)
            l.childRecordConflict(conflicts);
        clear();
    }
