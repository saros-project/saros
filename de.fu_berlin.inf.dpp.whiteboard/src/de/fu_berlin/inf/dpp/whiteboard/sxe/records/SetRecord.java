 ...
 * 
 * <p>
 * If the version is left free it is automatically set to target.getVersion()+1
 * when applying. This helps to ensure that locally created set-records can
 * always be applied.
 * </p>
 * 
 * @author jurke
...
public class SetRecord extends AbstractRecord {
	...
	public String toString() {
		...
		if (getParentToChange() != null)
			sb.append(" parent=" + getParentToChange().getRid());
		if (getPrimaryWeight() != null)
	...
	/**
	 * Utility for local records whether they change anything and should be
	 * applied
	 * 
	...
	public boolean changesTargetState() {
		...
		if (parentToChange != null
				&& !parentToChange.equals(target.getParent()))
			return true;
	...
	public boolean setsAllMutableFields() {
		if (chdata == null && target.getNodeType() == NodeType.ATTR)
			return false;
		if (parentToChange == null)
			return false;
		if (primaryWeight == null)
	...
	/**
	 * Utility method to fill all this local fields that are empty from the
	 * provieded SetRecord
	 */
	...
	public boolean equals(SetRecord r) {
		...
		if (chdata != null)
			if (!(chdata.equals(r.getChdata())))
				return false;

		if (parentToChange != null)
			if (!(parentToChange.equals(r.getParentToChange())))
				return false;
	...
	public boolean canApply() {
		try {
			...
			if (!getTarget().getDocumentRecord().contains(target))
				return false;
			if (target.getSetRecords().contains(this))
				return false;
			if (version == -1)
				return true;
			else
				return version == target.getVersion() + 1;
