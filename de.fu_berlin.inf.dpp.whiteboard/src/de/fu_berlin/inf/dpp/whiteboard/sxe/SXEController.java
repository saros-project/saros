 ...
 * <ul>
 * <li>maintain state of the controller during session negotiation</li>
...
public class SXEController extends AbstractSXEMessageHandler {
     ...
     */
    protected MappedList<RecordDataObject> unappliedMissingTargetRecords = new MappedList<RecordDataObject>();
    /**
    ...
    /**
     * Accepts a remote state and applies it to this controller, discarding the
     * previous content. </br>
     * 
    ...
    public void startSession(SXEMessage message) {
        ...
        if (unappliedBigVersionSetRecords.size() != 0)
            log.warn(prefix()
                + "Some set records haven't been applied during start synchronization");

        if (this.unappliedMissingTargetRecords.size() != 0)
            log.warn(prefix()
                + "Some records couldn't be inserted during start synchronization");
    ...
    /**
     * This method attempts to convert a RecordDataObject to the corresponding
     * record that will be applied then.</br>
     * 
     * If a RecordDataObject is not casual ready (parent, target or predecessor
     * SetRecord is missing), it will be queued. The only exception are
     * SetRecords during start synchronization where missing records (==version)
     * are expected due to possible previous conflicts.
     * 
     * @param rdo
    ...
    protected boolean apply(RecordDataObject rdo) {
        try {
            ...
            if (rdo.isAlreadyApplied(document)) {
                log.debug("Doubled " + rdo.getRecordType()
     ...
     * 
     * <p>
     * This may be expected behavior:</br>
     * 
     * During start synchronization SetRecords may be missing (received version
     * is too big) because they were discarded remotely due to previous
     * conflicts.</br>
     * 
     * In a session, there may arise conflicts (received version is too small).<br>
     * 
     * Everything else is an error, while no action has to be taken during start
     * synchronization but remote records must be queued to wait for the
     * SetRecord with the missing version to arrive.
     * </p>
     * 
     * 
     * @param r
     * @param startSync
     * @return whether the difference is handled and the record should not be
     *         applied
     */
    protected boolean handleVersionDiff(SetRecord r, boolean startSync) {
        ...
        if (versionDiff < 1) {
            ...
            if (startSync) {
                 ...
                 */
                log.debug("Set record with out-of-order version. Should be "
                    + (r.getTarget().getVersion() + 1) + " but is "
                    + (r).getVersion() + ", set-record: " + r
                    + ". Queued for the moment.");
    ...
    protected void applyQueuedRecords(IRecord cause) {
        if (cause.getRecordType() == RecordType.NEW) {
            ...
            List<RecordDataObject> rdos = unappliedMissingTargetRecords
    ...
    /**
     * Attempts to apply the passed record. If successful it will apply all
     * queued records that became casual ready now.
     * 
     * @param record
     ...
     * 
     * @param message
     *            message containing records to apply
     */
     ...
     * 
     * <p>
     * It will apply them, notify local listeners and if applying was successful
     * transmit them to the peers.
     * </p>
     * 
     * @param records
     */
    ...
    /**
     * Attempts to change the state to {@link State#CONNECTING}. If successful,
     * this will initialize to receive remote records from NOW.
     * 
     * @param session
    ...
    public boolean switchToConnectingState(SXESession session) {
        if (connectionState != State.INIT) {
            log.debug("Cannot change from " + connectionState + " to "
                + State.CONNECTING);
            return false;
        }
        log.debug("Whiteboard state changed from " + connectionState + " to "
            + State.CONNECTING);
        connectionState = State.CONNECTING;
