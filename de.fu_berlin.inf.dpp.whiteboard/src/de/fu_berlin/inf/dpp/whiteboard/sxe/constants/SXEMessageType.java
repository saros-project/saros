 ...
 * 
 * <p>
 * Each constant but {@link #RECORDS} is enhanced by its XML tag to be used when
 * serialized. Accessible by the toString()-method.</br>
 * 
 * The static utility method fromString(String) facilitates deserialization. It
 * also checks whether the provided tag is a {@link RecordType} and will return
 * {@link #RECORDS} respectively.
 * </p>
 * 
 * @author jurke
 * 
...
public enum SXEMessageType {
	...
	RECORDS("records"), STATE_OFFER("state-offer"), REFUSE_STATE("refuse-state"), ACCEPT_STATE(
			"accept-state"), ACK_STATE("ack-state"), STATE("state");
	...
	public static SXEMessageType fromString(String raw) {
		if (raw.equals(RECORDS.toString()))
			return null;
		for (SXEMessageType t : SXEMessageType.values())
			if (t.toString().equals(raw))
				return t;
		if (RecordType.fromString(raw) != null)
			return RECORDS;
		return null;
	}
