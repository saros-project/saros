...
/**
 * <p>
 * Implementation of a DOM element in context of Shared XML Editing XEP-0284
 * (SXE).
 * </p>
 * <p>
 * It extends the {@link NodeRecord} functionality by an attribute and child
 * element hierarchy plus respective utility methods and notification.
 * </p>
 * 
 * 
 * @author jurke
...
public class ElementRecord extends NodeRecord {
    ...
    /**
     * Used to notify listeners only after a whole SXECommand/SXEMessage was
     * executed.
     * 
    ...
    protected List<NodeRecord> getAllDescendantNodes() {
        ...
        for (ElementRecord r : getChildElements())
            records.addAll(r.getAllDescendantNodes());
     ...
     * 
     * @param key
     * @return parsed integer from chdata attribute value of the child
     *         AttributeRecord with name key
     * @throws NullPointerException
     *             , NumberFormatException respective Integer.parseInt()
     */
    public int getAttributeInt(String key) throws NullPointerException,
    ...
    public ElementRecord createNewElementRecord(String ns, String name) {
        ElementRecord record = getDocumentRecord()
            .createElementRecord(ns, name);
     ...
     * 
     * With "withoutSetRecords" set to true it has the same behavior as
     * {@link #createNewAttributeRecord(String, String, String)}. </br>
     * 
     * Set to false: It returns a SetRecord if an AttributeRecord for the
     * provided name exists already, else a new AttributeRecord is returned.
     * </p>
     * 
     * @param onlyCreateNewRecords
     *            whether to return a SetRecord if an AttributeRecord for the
     *            provided name exists already or not to do it.
     * @return A AttributeRecord or SetRecord respectively
     */
    public IRecord createNewOrSetAttributeRecord(String ns, String name,
        String chdata, boolean onlyCreateNewRecords) {
        AttributeRecord r = attributes.get(name);
        if (r == null || onlyCreateNewRecords)
            return createNewAttributeRecord(ns, name, chdata);
    ...
    /**
     * Returns the AttributeRecord created by DocumentRecord with this as
     * parent.
     * 
     * @see de.fu_berlin.inf.dpp.whiteboard.sxe.records.DocumentRecord#createAttributeRecord(String,
     *      String)
     */
    public AttributeRecord createNewAttributeRecord(String ns, String name,
        String chdata) {
        AttributeRecord record = getDocumentRecord().createAttributeRecord(ns,
            name, chdata);
        record.setParent(this);
        return record;
    }
    ...
    public boolean apply(DocumentRecord document) {
        if (getParent() == null && !document.isEmpty())
            return false; // has neither parent nor is root
        ...
        if (!document.isEmpty()) {
             ...
             * contained
             */
            if (getDocumentRecord().contains(this))
                return false;
            getParent().add(this);
    ...
    /**
     * <p>
     * Removes a child node from its set and adds it again respective its
     * primary-weight.
     * </p>
     * <p>
     * This has to be done if the primary-weight has changed.
     * </p>
     * 
     * @param child
     */
     ...
     * 
     * Usage: to remove all child references on uncommitted records
     */
    @Override
    public void clear() {
        super.clear();
    ...
    public ElementRecord getCopy() {
        ElementRecord copy = getDocumentRecord().createElementRecord(getNs(),
            getName());
    ...
    /**
     * 
     * Note: this method returns an element that contains copies of all
     * descendants of this record.
     * 
     * @param deep
     *            whether to include child nodes
     * @return if deep==false same as {@link #getCopy()} else the copy is
     *         extended by a copy of the subtree of child nodes
     */
    ...
    /**
     * Note, all returned records don't have children but applying would create
     * a copied hierarchy of this one's descendants to the passed ElementRecord
     * parent.
     * 
     * @param parent
     * @return a list with copies of all child nodes with <code>parent</code> as
     *         new parent, plus recursively copies of the whole subtree.
     */
     ...
     */
    public void notifyChildConflict(NodeRecord child, SetRecord previousState,
        SetRecord conflictSetRecord) {
        changeSupport.addChildRecordConflict(child, previousState,
            conflictSetRecord);
    }
    ...
    public void removeChildRecordChangeListener(
        ChildRecordChangeListener listener) {
