...
public class ScalingTest {
	...
	private static ChangeBoundsRequest createRequest(EditPart child,
			int direction, double sizeDeltaX, double sizeDeltaY) {
	...
	private static GraphicalEditPart createEditPart(final IFigure partFigure) {
		GraphicalEditPart editPart = new AbstractGraphicalEditPart() {
	...
	public void setup() {
		...
		for (Method m : c.getDeclaredMethods()) {
			if (m.getName().equals(resizeDataMethodName))
				recalculateResizeData = m;
		}
		assertNotNull("wrong method name for recalculating delta values",
				recalculateResizeData);
	...
	public void scalingCalculation() {
		...
		double[][] deltaValues = { { 100, 0 }, { 0, 100 }, { 100, 100 },
				{ 0, 0 }, { 50, 50 }, { 20, 20 }, { 33, 66 }, { 44.4, 4 },
				{ -101, -1 } };
		int[] directions = { PositionConstants.SOUTH,
				PositionConstants.SOUTH_WEST, PositionConstants.WEST,
				PositionConstants.NORTH_WEST, PositionConstants.NORTH,
				PositionConstants.NORTH_EAST, PositionConstants.EAST,
		...
		for (int direction : directions) {
			for (double[] sizeDelta : deltaValues) {
				...
				request = createRequest(child, direction, sizeDeltaX,
						sizeDeltaY);
				// run the recalculation method with the given parameters
				// altering request data
				try {
					recalculateResizeData.invoke(resizableEditPolicy, request,
							child);
				} catch (Exception e) {
					e.printStackTrace();
					fail(e.getMessage());
				 ...
				 */
				// desired values
				int deltaWidth = (int) (sizeDeltaX * child.getFigure()
						.getSize().width) / 100;
				int deltaHeight = ((int) (sizeDeltaY * child.getFigure()
						.getSize().height) / 100);
				// checking actual values
				Assert.assertEquals("wrong percentage calculation (width)",
						deltaWidth, request.getSizeDelta().width);
				Assert.assertEquals("wrong percentage calculation (height)",
						deltaHeight, request.getSizeDelta().height);
				/*
				 * move section
				 */
				// actual values
				...
				case PositionConstants.WEST:
					Assert.assertEquals("wrong positioning (WEST)", moveX,
							-deltaWidth);
					break;
				// NORTH direction: y adjustment
				case PositionConstants.NORTH_EAST:
				case PositionConstants.NORTH:
					Assert.assertEquals("wrong positioning (NORTH)", moveY,
							-deltaHeight);
					break;
