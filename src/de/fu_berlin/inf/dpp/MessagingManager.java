...
public class MessagingManager implements PacketListener, IConnectionListener {
     ...
     * and reopened again.
     */
    public class Session implements SessionProvider, PacketListener{
        private String          name;
        private Chat            chat;
        ...
        private String          lastRemotePacketID;
        
        public Session(Chat chat, String name) {
            this.chat = chat;
            this.name = name;
         ...
         */
        public JID getParticipant() { // HACK
            return participant;
        }
        ...
        public void processPacket(Packet packet) {
            ...
            Display.getDefault().syncExec(new Runnable() {
                public void run() {
                    ...
                    lastRemotePacketID = message.getPacketID();
        ...
        /* 
         * @see de.fu_berlin.inf.dpp.MessagingManager.SessionProvider#sendMessage(java.lang.String)
         */
     ...
     * only to handle incoming chat messages for which no sessions have been
     * yet created. If it finds a chat message that belongs to no current
     * session, it creates a new session.
     * 
    ...
    public void processPacket(Packet packet) {
        ...
        Display.getDefault().syncExec(new Runnable() {
            public void run() {
                try {
                    Session session = showMessagingWindow(jid, message.getThread());
    ...
    /**
     * @param thread ID of thread or <code>null</code> if chat should start a
     * new thread.
     * @throws XMPPException 
     */
    public Session showMessagingWindow(JID remoteUser, String thread) throws XMPPException {
        if (!Saros.getDefault().isConnected())
            throw new XMPPException("No connection");
        
        for (Session session : sessions) {
            if (remoteUser.equals(session.getParticipant())) {
                // HACK reopen window if not open so far
                session.openWindow();
                return session;
            }
        }
        ...
        Chat chat = (thread != null) ? 
            new Chat(connection, remoteUser.getJID(), thread) :
            new Chat(connection, remoteUser.getJID());
        ...
        Session session = new Session(chat, name);
