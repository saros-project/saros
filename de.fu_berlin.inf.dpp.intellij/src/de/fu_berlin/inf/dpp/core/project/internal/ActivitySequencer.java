...
public class ActivitySequencer implements Startable {
    ...
    private static final Logger LOG = Logger
        .getLogger(ActivitySequencer.class.getName());
    ...
    private final Runnable activitySender = new Runnable() {
        ...
        public void run() {
            ...
            while (true) {
                ...
                for (Map.Entry<JID, SequencedActivities> e : activitiesToSend
                    .entrySet()) {
    ...
    public void start() {
        ...
        activitySendThread = ThreadUtils
            .runSafeAsync("ActivitySender", LOG, activitySender);
    }
    ...
    public void stop() {
        ...
        try {
            ...
            LOG.warn(
                "interrupted while waiting for " + activitySendThread.getName()
                    + " thread to terminate"
            );
        ...
        if (activitySendThread.isAlive()) {
            LOG.error(
                activitySendThread.getName() + " thread is still running");
        }
     ...
     */
    private void executeActivity(JID sender,
        SequencedActivity sequencedActivity) {
        ...
        synchronized (bufferedIncomingActivities) {
            ...
            if (buffer == null) {
                LOG.warn("dropping received activity from " + sender
            ...
            while (true) {
                ...
                for (Iterator<SequencedActivity> it = buffer.activities
                    ...
                    if (sequencedActivity.sequenceNumber
    ...
    public void sendActivity(List<User> recipients, final IActivity activity) {
        ...
        for (User user : recipients) {
            ...
            LOG.trace(
                "dispatching activity " + activity + " to the local user: "
                    + user.getJID()
            );
        ...
        synchronized (bufferedOutgoingActivities) {
            for (User recipient : remoteRecipients) {
                ...
                if (buffer == null) {
                    LOG.warn("cannot send activity to " + recipient
                        + " because it is currently not registers, dropped activity: "
                        + activity);
                    continue;
                }
        ...
        synchronized (bufferedIncomingActivities) {
            if (bufferedIncomingActivities.get(user.getJID()) == null) {
                bufferedIncomingActivities.put(user.getJID(),
                    new ActivityBuffer<SequencedActivity>(FIRST_SEQUENCE_NUMBER)
                );
            }
        }
    }
    ...
    public void flush(User user) {
        ...
        synchronized (bufferedOutgoingActivities) {
            while (true) {
                ...
                if (buffer == null || (buffer.activities.size() == 0
                    && !buffer.isInTransmission)) {
                    break;
                }
    ...
    private void receiveActivities(Packet activityPacket) {
        ...
        String msg =
            "rcvd (" + String.format("%03d", activities.size()) + ") " + from
                + ": " + activities;
        ...
        for (IActivity activity : activities) {
            ...
            assert activity.getSource() != null :
                "received activity without source" + activity;

            executeActivity(from,
                new SequencedActivity(activity, sequenceNumber++));
        }
    }
