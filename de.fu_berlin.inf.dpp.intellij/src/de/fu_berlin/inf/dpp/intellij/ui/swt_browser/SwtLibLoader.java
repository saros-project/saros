...
public class SwtLibLoader {
    ...
    /**
     * This methods adds the SWT library to the plugin class loader
     * responsible for the Saros IntelliJ plug-in.
     * <p/>
     * For this to work the SWT libary plugin has to be installed in IntelliJ.
     * Currently Windows, Mac OS, and Linux is supported, both 32 and 64 bit.
     * <p/>
     * This method should only be called once.
     *
     * @throws IllegalStateException if the actual OS arch combination
    ...
    private static PluginClassLoader obtainPluginClassLoader() {
        ...
        if (!(classLoader instanceof PluginClassLoader)) {
            throw new RuntimeException(
                "unable to get hold of the plugin classloader");
        }
    ...
    private static URL getSwtLibUrl() {
        ...
        String swtJarPath =
            pluginsPath + "/de.fu_berlin.inf.dpp.swt_plugin/resources/"
                + jarFilename;
        File jarFile = new File(swtJarPath);
        if (!jarFile.exists()) {
            throw new IllegalStateException(jarFile.getAbsolutePath()
    ...
    static String getJarFilename(String osName, String osArch) {
        return JAR_BASE_FILENAME + getOsSuffix(osName) + "-" + getArchSuffix(
            osArch) + ".jar";
    }
    ...
    private static String getArchSuffix(String osArch) {
        if (osArch.contains("64")) {
            ...
            throw new IllegalStateException(
                "Your OS architecture: " + osArch + " is not supported.");
        }
    }
    ...
    private static String getOsSuffix(String osName) {
        if (osName.contains("win")) {
            ...
            throw new IllegalStateException(
                "Your operating system: " + osName + " is not supported.");
        }
    }
}