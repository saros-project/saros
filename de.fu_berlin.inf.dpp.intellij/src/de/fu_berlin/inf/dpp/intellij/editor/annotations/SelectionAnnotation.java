...
/**
 * Represents a Saros selection annotation. Selection annotations are used
 * during a session to highlight the text sections that are currently selected
...
class SelectionAnnotation extends AbstractEditorAnnotation {
     ...
     * <p>
     * A selection annotation must have exactly one annotation range.
     * </p>
     *
    ...
    SelectionAnnotation(
        ...
        if (annotationRanges.size() != 1) {
            throw new IllegalArgumentException(
                "A selection annotation must have exactly one "
                    + "AnnotationRange.");
        }
    }
}