...
public class TestXMPPFileTransferSender extends TestCase implements
	PacketListener {
	...
	// transferManager.getProperties().setProperty(Socks5TransferNegotiatorManager.PROPERTIES_PORT,"50010");transferManager.getProperties().setProperty(IBBTransferNegotiator.PROPERTIES_BLOCK_SIZE,
	// "40690");
	...
	OutgoingFileTransfer transfer = transferManager
		.createOutgoingFileTransfer(RECEIVER_JID + RESOURCE);
	...
	if (!packet.getFrom().equals(connection.getUser())) {
	    ...
	    /*
	     * 1. überprüfen ob es eine subscribe anfrage ist 2. überprüfen,
	     * ob user im in liste ist. 3. hinzufügen des accounts.
	     */
		...
		if (p.getType().equals(Presence.Type.available)) {
		    ...
		    if (StringUtils.parseBareAddress(p.getFrom()).equals(
			    RECEIVER_JID)) {
			logger.info("Presence RECEIVER_JID " + p.getFrom()
				+ " " + p.getType());
			send = true;
			return;
		    }
