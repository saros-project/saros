...
public class SimpleServerProxyTest extends JupiterTestCase {
	...
	/*
	 * TODO: Die Operation wird schon beim Server umgewandelt und muss auf
	 * Client-Seite einfach nur entsprechnend ausgeführt werden.
	 */
	...
	assertEquals(c1.getDocument(), c2.getDocument());
	System.out
		.println("END OF METHOD: testThreeConcurrentInsertOperations");
    }
	...
	// $JUnit-BEGIN$
	suite.addTest(new SimpleServerProxyTest(
		"testTwoConcurrentInsertOperations"));
	suite
		.addTest(new SimpleServerProxyTest(
			"testTwoClientWithJupiterProxy"));
	suite.addTest(new SimpleServerProxyTest(
		"testThreeConcurrentInsertOperations"));
	suite.addTest(new SimpleServerProxyTest(
		"testTwoClientWithJupiterProxyDeleteInsertOperations"));
	// $JUnit-END$
	return suite;
    }
