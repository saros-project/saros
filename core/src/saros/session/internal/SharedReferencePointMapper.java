...
class SharedReferencePointMapper {
  ...
  public SharedReferencePointMapper() {
    idToReferencePointMapping = new HashMap();
    referencePointToIDMapping = new HashMap();
    referencePointsOfUsers = new HashMap();
    partiallySharedResourceMapping = new HashMap();
    completelySharedReferencePoints = new HashSet();
    partiallySharedReferencePoints = new HashSet();
  }
  ...
  public synchronized void addReferencePoint(
    ...
    if (isPartially) partiallySharedResourceMapping.put(referencePoint, new HashSet());
  ...
  public synchronized void removeReferencePoint(String id) {
    ...
    if (referencePoint == null) {
      LOG.warn("could not remove referencePoint, no referencePoint is registered with ID: " + id);
      return;
    }
  ...
  public synchronized void addResources(
    ...
    if (partiallySharedResources.isEmpty()) {
      partiallySharedResources = new HashSet(Math.max(1024, (resources.size() * 3) / 2));
  ...
  public synchronized List<IResource> getPartiallySharedResources() {
    ...
    List<IResource> partiallySharedResources = new ArrayList(size);
  ...
  public synchronized Map<IReferencePoint, List<IResource>> getReferencePointResourceMapping() {
    ...
    Map<IReferencePoint, List<IResource>> result = new HashMap();
    ...
    for (Map.Entry<IReferencePoint, Set<IResource>> entry :
      ...
      if (entry.getValue() != null) partiallySharedResources = new ArrayList(entry.getValue());
  ...
  public synchronized void addMissingReferencePointsToUser(User user) {
    List<String> referencePoints = new ArrayList(idToReferencePointMapping.keySet());
