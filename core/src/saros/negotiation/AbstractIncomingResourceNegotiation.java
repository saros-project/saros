...
/**
 * Handles incoming ResourceNegotiations except for the actual file transfer.
 *
...
public abstract class AbstractIncomingResourceNegotiation extends ResourceNegotiation {
  ...
  public Status run(
    ...
    try {
      ...
      List<FileList> missingFiles =
          synchronizeReferencePointStructures(
      ...
      /*
       * the user who sends this ResourceNegotiation is now responsible for the
      ...
      for (Entry<String, IReferencePoint> entry : referencePointMapping.entrySet()) {
        ...
        /*
         * TODO Queuing responsibility should be moved to ResourceNegotiation, since its the only
      ...
      /*
       * If we are the session's host and are receiving resources from a
       * non-host user, we need to notify all components that the user
       * already has these resources and can process (and send) activities
       * targeting them. Otherwise, activities generated while we are
       * still receiving the resource archive will get lost.
       *
       * FIXME: userStartedQueuing() needs a better name which is less
       * bound to its use in OutgoingResourceNegotiation.
       */
      if (session.isHost()) {
      ...
      /*
       * We are finished with the negotiation. Add all reference point resources
       * to the session.
       */
      for (Entry<String, IReferencePoint> entry : referencePointMapping.entrySet()) {
  ...
  /**
   * In preparation of the ResourceNegotiation, this setups a File Transfer Handler, used to receive
  ...
  protected void executeCancellation() {
     ...
     * the reference point is marked as shared on the remote side and so will never
     * be able to be shared again to us. Again the whole architecture does
     * currently NOT support cancellation of the resource negotiation
     * properly !
     */
   ...
   * @param monitor the progress monitor
   * @return list of differences (one for each reference point) between the local and the remote
   *     side.
   * @throws SarosCancellationException
   * @throws IOException
   */
  protected Map<String, FileListDiff> computeLocalVsRemoteDiff(
    ...
    for (final Entry<String, IReferencePoint> entry : localReferencePointMapping.entrySet()) {
      ...
      final FileList localReferencePointFileList =
  ...
  /**
   * Synchronize the reference point structures, deleting files and folders that are not present on
   * the remote side and creating empty folders that do not exists and the local side.
   *
   * @param localReferencePointMapping
   * @param diffs
   * @return list of file lists (each for every reference point) containing the missing files that
   *     are not present on the local side.
   * @throws IOException
   */
  protected List<FileList> synchronizeReferencePointStructures(
    ...
    for (final Entry<String, IReferencePoint> entry : localReferencePointMapping.entrySet()) {
      ...
      for (final String path : diff.getAddedFolders()) {
        final IFolder folder = referencePoint.getFolder(path);
  ...
  protected void checkReferencePointMapping(final Map<String, IReferencePoint> mapping) {
  ...
  protected IResource getResource(IReferencePoint referencePoint, String path) {
    if (path.endsWith(FileList.DIR_SEPARATOR)) return referencePoint.getFolder(path);
    else return referencePoint.getFile(path);
  }
