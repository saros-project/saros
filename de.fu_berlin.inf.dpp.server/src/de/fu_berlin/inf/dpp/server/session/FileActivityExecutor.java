 ...
 */
public class FileActivityExecutor extends AbstractActivityConsumer implements
    Startable {

    private static final Logger LOG = Logger
        .getLogger(FileActivityExecutor.class);
     ...
     *            the current session
     * @param editorManager
     *            the editor manager to update the file mapping
     *            on a file move
     */
    public FileActivityExecutor(ISarosSession session,
    ...
    private void executeFileMove(FileActivity activity) throws IOException {
        ...
        if (content != null) {
            newFile.setContents(new ByteArrayInputStream(content),
                true, true);
        }
