...
/**
 * Server implementation of the {@link IContainer} interface. Every type of
 * container is implemented as a simple directory in the file system.
 */
public abstract class ServerContainerImpl extends ServerResourceImpl implements
    IContainer {
    ...
    public void move(IPath destination, boolean force) throws IOException {
        IPath destinationBase = destination.isAbsolute() ? getWorkspace()
            .getLocation() : getLocation().removeLastSegments(1);

        IPath absoluteDestination = destinationBase.append(destination);
        FileUtils.moveDirectory(getLocation().toFile(),
            absoluteDestination.toFile());
    }
