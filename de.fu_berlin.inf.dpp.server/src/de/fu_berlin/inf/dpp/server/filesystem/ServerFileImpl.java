...
public class ServerFileImpl extends ServerResourceImpl implements IFile {
    ...
    /**
     * Sets the character encoding to use when decoding this file to text.
     * Passing <code>null</code> means the default encoding of the surrounding
     * container should be used (which is the default behavior).
     * 
     * @param charset
    ...
    public void delete(int updateFlags) throws IOException {
        try {
            ...
            LOG.debug("Could not delete " + getFullPath()
                + " because it doesn't exist (ignoring)", e);
        }
    }

    @Override
    public void move(IPath destination, boolean force) throws IOException {
        IPath destinationRoot = destination.isAbsolute() ? getWorkspace()
            .getLocation() : getLocation().removeLastSegments(1);

        IPath absoluteDestination = destinationRoot.append(destination);
        Path nioDestination = ((ServerPathImpl) absoluteDestination)
            .getDelegate();
    ...
    public void setContents(InputStream input, boolean force,
        ...
        if (input != null) {
            Files
                .copy(input, tempFilePath, StandardCopyOption.REPLACE_EXISTING);
        }

        try {
            Files.move(tempFilePath, toNioPath(),
