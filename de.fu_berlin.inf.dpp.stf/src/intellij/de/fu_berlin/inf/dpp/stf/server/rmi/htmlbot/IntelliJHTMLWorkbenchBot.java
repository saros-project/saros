...
/**
 * The IntelliJ implementation of the {@link IHTMLWorkbenchBot}.
 * It is used for opening and closing the browser tool window.
 */
public class IntelliJHTMLWorkbenchBot implements IHTMLWorkbenchBot {
    ...
    public boolean isSarosBrowserViewOpen() throws RemoteException {
        try {
            return robot.finder().find(new TypeMatcher(SwtBrowserPanel.class))
                .isShowing();
    ...
    private void toggleBrowserView() {
        JMenu viewMenu = robot.finder()
            .find(new GenericTypeMatcher<JMenu>(JMenu.class) {
                @Override
