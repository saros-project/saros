package de.fu_berlin.inf.dpp.stf.selftest.contextmenu;
...
public class ContextMenuRunAsTest extends StfTestCase {
    ...
    public void runAsJavaApplicationTest() throws Exception {
        ...
        ALICE.superBot().internal()
            .createFile("foo", "src/foo/Foo.java", createJavaClassContent());

        IContextMenusInPEView javaClass = ALICE.superBot().views()
            .packageExplorerView().selectFile("foo", "src", "foo", "Foo.java");

        IRunAsContextMenu runAs = javaClass.runAs();
        runAs.javaApplication();

        ALICE.superBot().views().consoleView()
            .waitUntilCurrentConsoleContainsText("Hello World 4");
    ...
    private String createJavaClassContent() {
        ...
        builder
            .append("for(int i = 0; i < 20; System.out.println(\"Hello World \" + (i++)));");
        builder.append("}}");
